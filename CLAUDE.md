...
- Enforced by [worker/tests/exchange-rate-feed.test.ts](worker/tests/exchange-rate-feed.test.ts): a currency-feed host anywhere under `src` fails it, as does a second table of rates, a re-gated refresh, `useShopPrice` ceasing to read the shared table, or a fallback that invents a number.
