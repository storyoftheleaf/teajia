...
- Enforced by [worker/tests/a-tea-arrives-with-its-cost.test.ts](worker/tests/a-tea-arrives-with-its-cost.test.ts), which pins the rule, checks every door goes through it, checks the agent door asks the RAW argument before any coercion, and reads the form. It strips comments before scanning, because a guard that reads its own explanation can only be satisfied by deleting the reason it exists.
