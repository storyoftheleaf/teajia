...
- The rail, top to bottom: emblem, then `read / craft / advise / shop` as lowercase Cormorant at `text-ui-14`, a hairline and `manage` as a fifth word when the signed-in account has a Manage room, a spacer, then the foot: three glyphs (search, the person for Your Table, the bag for cart with a gold count), the same three the mobile bar and page headers use, then two words at `text-ui-12`: people (the directory at `/people`) and places (`/spaces`, until the connections page, people and places merged with Find a Table, is decided; the word is Places, never Connections). Settings left the foot on 2026-09-22 and is a room in the Manage column; the light/dark control lives in the Your Table header, not on the rail. Adrian asked for the foot to be glyphs (2026-09-22): words there were the one thing on the rail that did not match the bar. Words are written capitalised in the markup and lowercased by CSS so accessible names stay `Read`, `Your Table`, which the browser specs match exactly. The words sit on a 34px pitch, clumped as drawn on the canvas: never put `tap-target` on them, it makes each word a 44px box and spreads the column down the whole edge (this happened, 2026-09-21). `.nav-rail-word` grows the hit area by a pseudo-element instead, the full rail width and half the gap above and below.
...
- **The bottom bar has two doors (2026-09-22).** Its left end is a three-line glyph in the same 1.6 to 1.75 stroke as the person glyph on the right, so the two ends mirror. It raises the site panel (`src/components/SiteMenu.tsx`) the way the right end raises Your Table: search first, then sessions, people, places, tea wisdom, cart, about (2026-09-23: About had no door but the footer), and for anyone with a Manage room a hairline and the Manage rooms. A customer never sees the second band. Search left the bar to make the slot; it is first in the panel and in every page header. The Manage rooms come from one list, `src/components/manageNav.ts`, read by both the desktop column and the panel, so a room added there appears on both under the same word; Members & Access is in that list because it was reachable only through a Your Table tile before. One word per route (2026-09-23): Business and its Activity child became one room, Orders, gated on sell like its route; People is its own room with Tea Masters under it; Tasting Notes sits under Magazine; Equipment, Collection and Sources left the column because their routes only redirect. The phone's admin bar reads its words from `src/components/adminBarTabs.ts`, and `src/components/manageNav.oneWord.test.ts` fails if either list gives a route a second word. The bar keeps its seven slots and its widths at 375px; do not put a word in the left slot, it unbalances the capsule (measured 2026-09-22).
