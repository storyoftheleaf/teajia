...
- **Desktop (redesigned 2026-09-21, round two of a six-direction canvas):** LeftSidebar is a 72px rail of words flush to the left edge, and a 200px Manage column beside it that exists only while Manage is the room. The rail wears `.nav-rail` in `card-utilities.css`: the page ground lifted a few percent of tone (`color-mix` of `--tea-surface` into `--tea-bg-rgb`) and one keyline on its right edge. No pod, no blur, no paper, no shadow, no radius, no icons. It replaced two floating pods cut from the mobile bar's material, which on a wide dark page read as objects placed on the page rather than part of it; depth now comes from tone, never effects. The `.nav-pod` recipe is gone with them.
- The rail, top to bottom: emblem, then `read / craft / advise / shop` as lowercase Cormorant at `text-ui-14`, a hairline and `manage` as a fifth word when the signed-in account has a Manage room, a spacer, then the foot words at `text-ui-12`: search, your table, cart (gold count), connections, settings when the account has one, and `light` / `dark` (the theme control is the word for the mode it switches to). Words are written capitalised in the markup and lowercased by CSS so accessible names stay `Read`, `Your Table`, which the browser specs match exactly.
- The Manage column keeps the manage nav exactly as it was: 42px rows, `text-ui-16` words, the children column under the active parent. Clicking a browse word puts the room back to browse and the column goes; landing on an admin route opens it. There is no collapse toggle any more: the rail is already the compact state (`sidebarCollapsed` in the store is unused).
- The aside width is one CSS variable, `--teajia-sidebar-w` (`4.5rem` rail alone, `17rem` with the column), set by LeftSidebar. `App.tsx` reads it for the main column's left margin and `.sidebar-inset` reads it for full-screen panels, so nothing else needs to know which state the sidebar is in.
