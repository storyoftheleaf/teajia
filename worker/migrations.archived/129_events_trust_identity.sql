...
INSERT INTO migration_129_tasting_note_validation(violation)
...
DROP TABLE migration_129_tasting_note_validation;
...
SELECT
  ...
  ranked.is_favorite,
  ranked.created_at,
  'superseded_during_migration_129'
FROM (
  SELECT
    note.*,
    ROW_NUMBER() OVER (
...
WHERE id IN (
  ...
  WHERE archive_reason = 'superseded_during_migration_129'
);
