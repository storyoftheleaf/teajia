...
const handleCreateArticle: Handler = async (request, env) => {
  ...
  const subjectIds = Array.isArray(body.subject_ids) ? JSON.stringify(body.subject_ids) : '[]';
  ...
  await env.DB.prepare(
    ...
    subjectIds,
    body.pull_quote || null,
    body.pull_quote_subject || null,
  ).run();
...
const handleUpdateArticle: Handler = async (request, env, params) => {
  ...
  if (Array.isArray(body.subject_ids)) body.subject_ids = JSON.stringify(body.subject_ids);
...
// GET /api/people/:slug — single profile + the woven content (articles by,
//                         pull-quotes about, events hosted, host account).
//
// All public, no auth, no X-Teajia-Account header. The directory currently
// returns every published contributor across accounts; multi-tenancy on
// reads is deferred until a future per-account public storefront cut.
...
const handleGetPublicContributor: Handler = async (request, env, params) => {
  ...
  // Pull-quotes from articles where this contributor is the subject.
  // Up to two, scattered between sections by the page layout.
  ...
  // Articles where this contributor is a subject (interviewed about, profiled).
  // subject_ids is a JSON array of slugs; SQLite has no JSON contains operator,
  // so we LIKE-match the JSON-encoded form. Safe because slugs are
  // lowercase + hyphens only and subject_ids is always JSON-encoded by the writer.
  ...
  // Where to find them: the host account's storefront, when this
  // contributor is the public face of an account.
  ...
  // Products attributed to this contributor (Hands on section).
  ...
  // Seasonal stamp: pick one entry whose region appears in the contributor's
  // location_line and whose date range covers today. Rough heuristic — Adrian
  // controls the calendar and the location_line, so a substring match is
  // sufficient for v1.
