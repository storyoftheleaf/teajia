...
describe('migration 017 rehearsals', () => {
  it('boots the clean canonical schema through migration 123', () => withDatabase((database) => {
    ...
    expect(migrationNames.at(-1)).toBe('123_wisdom_verifications.sql');
  ...
  it('upgrades the production-shaped pre-017 schema through migration 123', () => withDatabase((database) => {
    ...
    expect(applied.at(-1)).toBe('123_wisdom_verifications.sql');
