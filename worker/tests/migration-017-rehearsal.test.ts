...
describe('migration 017 rehearsals', () => {
  it('boots the clean canonical schema through the latest migration', () => withDatabase((database) => {
    ...
    expect(migrationNames.at(-1)).toBe('127_secure_inquiry_tracking.sql');
  ...
  it('upgrades the production-shaped pre-017 schema through the latest migration', () => withDatabase((database) => {
    ...
    const appliedBeforeSales = applyTrackedMigrations(database, '126_tea_master_sales.sql');
    sqlite(database, `
    ...
    expect(appliedSales).toEqual(['126_tea_master_sales.sql', '127_secure_inquiry_tracking.sql']);
