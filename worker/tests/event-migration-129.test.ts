...
function applyMigration127(db: DatabaseSync): void {
  db.exec(migration('129_events_trust_identity.sql'));
}
...
describe('migration 129', () => {
    ...
    `).all()).toEqual([
      {
        ...
        archive_reason: 'superseded_during_migration_129',
      },
      {
  ...
  it('keeps canonical schema constraints, foreign keys, and indexes equivalent to migration 129', () => {
