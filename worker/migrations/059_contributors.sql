...
-- the public route ships. Title-cased display_name as a placeholder; Adrian
-- writes the real fields through the admin editor before flipping
-- is_published.
...
-- row. display_name is a title-cased placeholder; Adrian fills the real
-- fields through the admin editor.
--
-- SQLite doesn't have a built-in title-case function. We approximate with
-- INITCAP-style construction: replace hyphens with spaces, then upper-case
-- the first letter of each word using a small case expression. For the
-- seed, the exact casing is cosmetic — Adrian will overwrite it when
-- publishing the contributor.
...
SELECT
  a.author_id AS id,
  a.account_id,
  -- Replace hyphens with spaces. Casing stays as the slug had it; Adrian
  -- rewrites display_name when filling out the profile, so we don't try
  -- to be clever here.
