...
import { readFileSync, writeFileSync, readdirSync, mkdirSync, existsSync, mkdtempSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join, dirname, resolve } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';

const projectRoot = join(dirname(fileURLToPath(import.meta.url)), '..');
const root = process.env.WISDOM_ROOT ? resolve(process.env.WISDOM_ROOT) : projectRoot;
...
const researchBundle = {
  sources: readJson('research-sources.json'),
  citations: readJson('citations.json'),
  potentialProfiles: readJson('potential-profiles.json'),
};

async function loadResearchValidator() {
  const temp = mkdtempSync(join(tmpdir(), 'teajia-wisdom-validator-'));
  ...
  try {
    ...
    return await import(`${pathToFileURL(outfile).href}?build=${Date.now()}`);
  } finally {
    rmSync(temp, { recursive: true, force: true });
  }
}

const { validateResearchBundle } = await loadResearchValidator();
const researchErrors = validateResearchBundle(researchBundle, {
  cultivar: cultivars.map(({ story, ...entry }) => ({ ...entry, ...story })),
  region: regions,
  producer: producers,
  style: styles,
  mark: marks,
...
if (researchErrors.length) {
  throw new Error(`Tea research validation failed:\n${researchErrors.join('\n')}`);
}
...
const researchBanner = (count, noun) => `// Generated by scripts/build-wisdom.mjs from data/tea-wisdom-source. Do not edit by hand.
// ${count} ${noun}. Re-run the script after changing the source files.\n`;
...
writeFileSync(join(generated, 'researchSources.ts'),
  `${researchBanner(publicResearchSources.length, 'public research sources')}import type { PublicResearchSource } from '../types';\n\nexport const RESEARCH_SOURCES: PublicResearchSource[] = ${JSON.stringify(publicResearchSources, null, 2)};\n`);

writeFileSync(join(generated, 'citations.ts'),
  `${researchBanner(publicCitations.length, 'public citations')}import type { WisdomCitation } from '../types';\n\nexport const WISDOM_CITATIONS: WisdomCitation[] = ${JSON.stringify(publicCitations, null, 2)};\n`);

writeFileSync(join(generated, 'potentialProfiles.ts'),
  `${researchBanner(publicPotentialProfiles.length, 'potential profiles')}import type { WisdomPotentialProfile } from '../types';\n\nexport const WISDOM_POTENTIAL_PROFILES: WisdomPotentialProfile[] = ${JSON.stringify(publicPotentialProfiles, null, 2)};\n`);
...
console.log(`research        ${publicResearchSources.length} sources, ${publicCitations.length} citations, ${publicPotentialProfiles.length} potential profiles`);
