 ...
 * Tests and tooling may set WISDOM_EXPORT_ROOT to write every output artifact
 * into an isolated directory instead of the tracked public dataset.
 */
...
const configuredOutDir = process.env.WISDOM_EXPORT_ROOT;
const outDir = configuredOutDir ? resolve(configuredOutDir) : join(root, 'public/wisdom');
const stampPath = configuredOutDir
  ? join(outDir, 'datasetStamp.ts')
...
writeFileSync(
  ...
  toCsv(regions, ['id', 'name', 'country', 'province', 'altitude', 'climate', 'description'])
);
