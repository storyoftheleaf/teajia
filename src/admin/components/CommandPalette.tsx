      ...
      <div role="dialog" aria-modal="true" aria-label="Command palette" className="w-full max-w-xl bg-tea-surface border border-tea-border rounded-lg shadow-lg overflow-hidden" onClick={e => e.stopPropagation()}>
        <Command className="w-full" label="Global Command Menu">
          ...
          <Command.List className="max-h-[300px] overflow-y-auto p-2 custom-scrollbar">
            ...
            {searchQuery.trim() && (
              <>
                {filteredProducts.length > 0 && (
                  <Command.Group heading="Products" className="text-ui-10 uppercase tracking-display font-bold text-tea-text-sec px-2 py-2">
                    {filteredProducts.map((p: any) => (
                      <Command.Item
                        ...
                        className="flex items-center gap-3 px-3 py-3.5 rounded-lg hover:bg-tea-elevated cursor-pointer text-tea-text aria-selected:bg-tea-elevated aria-selected:text-tea-text transition-colors"
                      >
                ...
                {filteredCustomers.length > 0 && (
                  <Command.Group heading="People" className="text-ui-10 uppercase tracking-display font-bold text-tea-text-sec px-2 py-2">
                ...
                {filteredEvents.length > 0 && (
                  <Command.Group heading="Events" className="text-ui-10 uppercase tracking-display font-bold text-tea-text-sec px-2 py-2">
            ...
            <Command.Group heading="Navigation" className="text-ui-10 uppercase tracking-display font-bold text-tea-text-sec px-2 py-2">
            ...
            {isDevAdmin && (
              <Command.Group heading="Admin" className="text-ui-10 uppercase tracking-display font-bold text-tea-text-sec px-2 py-2 mt-2 border-t border-tea-border">
                <Command.Item onSelect={() => runCommand(() => navigate('/admin/inventory'))} className="flex items-center gap-3 px-3 py-3.5 rounded-lg hover:bg-tea-surface cursor-pointer text-tea-text aria-selected:bg-tea-surface aria-selected:text-tea-gold transition-colors font-serif">
                  <Settings size={16} className="text-tea-text-sec" /> Master Inventory
                </Command.Item>
                <Command.Item onSelect={() => runCommand(onAddProduct)} className="flex items-center gap-3 px-3 py-3.5 rounded-lg hover:bg-tea-surface cursor-pointer text-tea-text aria-selected:bg-tea-surface aria-selected:text-tea-gold transition-colors font-serif">
