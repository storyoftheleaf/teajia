    ...
    <div className="fixed inset-0 z-modal flex items-center justify-center bg-tea-text/80 backdrop-blur-sm p-4" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label="Add to order" className="bg-tea-surface border border-tea-border rounded-lg w-full max-w-sm p-8 shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-tea-text-dim mb-2">Quantity ({product.type === 'Teaware' ? 'Units' : 'Grams'})</label>
            <input 
              ...
              className="w-full bg-tea-surface border border-tea-border rounded-lg p-3 text-tea-text focus:border-tea-text-dim outline-none text-lg transition-colors placeholder-tea-text-dim/50"
              placeholder="0"
            />
            {product.type !== 'Teaware' && (
                <div className="flex flex-wrap gap-2 mt-3">
                    {teaPresets.map(preset => (
                        <button key={preset.value} onClick={() => setQuantity(preset.value.toString())} className="px-3 py-1.5 bg-tea-surface text-tea-text-dim text-xs rounded-md border border-tea-border hover:bg-tea-border/50 hover:text-tea-text hover:border-tea-text-dim/50 transition-all num">
                            {preset.label}
                        </button>
                    ))}
                </div>
          ...
          <div className="flex justify-between items-center text-sm border-b border-tea-border pb-4">
            <span className="text-tea-text-dim">Price per unit:</span>
