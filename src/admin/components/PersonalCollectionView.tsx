...
export const PersonalCollectionView = ({ products, isLoading, onRefresh }: { products: Product[], isLoading: boolean, onRefresh: () => void }) => {
  ...
  const SortHeader = ({ colKey, label, align = 'left' }: { colKey: keyof Product, label: string, align?: 'left' | 'right' | 'center' }) => (
      <th 
        className={`py-3 px-4 cursor-pointer hover:text-tea-text transition-colors group select-none text-${align} border-b border-tea-border text-[10px] uppercase tracking-[0.2em] text-tea-muted`} 
  ...
  if (isLoading) {
    return <div className="p-12 text-center text-tea-muted flex justify-center items-center"><Loader2 className="animate-spin mr-2" /> Loading collection...</div>;
  }
    ...
    <div className="min-h-screen pb-24 md:pb-0 bg-tea-bg">
      ...
      <div className="sticky top-0 z-30 bg-tea-bg/90 backdrop-blur-md border-b border-tea-border py-4">
        <div className="px-4 md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-4">
            <div>
                ...
                <p className="text-tea-muted text-xs uppercase tracking-[0.2em] mt-1">Personal Holding • Not Listed in Catalog</p>
            </div>
      ...
      <div className="p-4 md:px-12 max-w-7xl mx-auto mt-6">
        {sortedProducts.length === 0 ? (
            <div className="py-24 text-center text-tea-muted border border-tea-border rounded-xl bg-tea-surface">
            ...
            <div className="rounded-xl border border-tea-border bg-tea-surface overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-tea-bg text-tea-muted font-serif uppercase tracking-[0.2em] text-xs border-b border-tea-border">
                            <tr>
                                ...
                                <th className="py-3 px-4 text-right cursor-pointer group select-none text-[10px] uppercase tracking-[0.2em] text-tea-muted">
                                ...
                                return (
                                    ...
                                    >
                                        <td className="py-2 px-4">
                                            ...
                                            <div className="text-[10px] text-tea-muted font-sans mt-0.5">
                                                <span>{product.givenName}</span>
                                            </div>
                                        </td>
                                        <td className="py-2 px-4">
                                            <span className="flex items-center gap-2 text-xs font-medium tracking-wide text-tea-muted">
                                                <span style={{ color: dotColor, fontSize: '10px' }}>●</span> {product.type}
                                            </span>
                                        </td>
                                        <td className="py-2 px-4 text-tea-muted text-xs font-serif italic">{product.year || 'N.V.'}</td>
                                        <td className="py-2 px-4 text-tea-muted text-xs">{product.vendor || 'Unknown'}</td>
                                        
                                        <td className="py-2 px-4 text-right font-mono text-xs text-tea-text">
                                            {product.stockGrams}g
                                        </td>
                                        
                                        <td className="py-2 px-4 text-right font-mono text-xs text-tea-muted">
                                        ...
                                        <td className="py-2 px-4 text-right">
                                            <button 
                                                ...
                                                className="opacity-0 group-hover:opacity-100 transition-opacity text-tea-muted hover:text-tea-text"
                                            >
                                                <Pencil size={14} />
                                            </button>
                                        </td>
