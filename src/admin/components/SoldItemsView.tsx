      ...
      <div className="flex flex-col md:flex-row justify-between items-end border-b border-tea-border pb-0 gap-4">
        ...
        <div className="flex gap-1">
             <button 
                ...
                className={`px-6 py-3 text-xs font-bold uppercase tracking-[0.2em] border-t border-l border-r rounded-t-lg transition-colors ${activeTab === 'archive' ? 'bg-tea-surface border-tea-border text-tea-text' : 'border-transparent text-tea-text-dim hover:text-tea-text'}`}
             >
             ...
             <button 
                ...
                className={`px-6 py-3 text-xs font-bold uppercase tracking-[0.2em] border-t border-l border-r rounded-t-lg transition-colors ${activeTab === 'logs' ? 'bg-tea-surface border-tea-border text-tea-text' : 'border-transparent text-tea-text-dim hover:text-tea-text'}`}
             >
                <div className="flex items-center gap-2">
                  ...
                  <div className="overflow-x-auto border border-tea-border rounded-xl bg-tea-surface">
                    ...
                    <tbody className="divide-y divide-tea-border">
                        ...
                        soldOutProducts.map(product => (
                            ...
                            <td className="p-4 text-tea-text-dim">{product.type}</td>
                            <td className="p-4 text-tea-text-dim">{product.vendor || '—'}</td>
                            <td className="p-4 text-right text-tea-text-dim num">{product.costPerGramUSD != null ? `$${fmtNum(product.costPerGramUSD)}` : '-'}</td>
                            <td className="p-4 text-right text-tea-text num">{product.pricePerGramUSD != null ? `$${fmtNum(product.pricePerGramUSD)}` : '-'}</td>
                            <td className="p-4 text-center"><span className="text-[10px] uppercase font-bold tracking-wider bg-tea-bg text-tea-text-dim border border-tea-border px-2 py-1 rounded-sm">Sold Out</span></td>
                            </tr>
                        ))
                        )}
                    </tbody>
