  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 print:bg-white print:p-0">
      <div className="bg-tea-bg border border-tea-border rounded-xl w-full max-w-sm p-8 shadow-2xl relative print:border-none print:shadow-none print:w-full print:max-w-none print:bg-white">
        <button onClick={onClose} className="absolute top-4 right-4 text-tea-muted hover:text-tea-text transition-colors print:hidden">
          <X size={20} />
        </button>
        
        <div className="text-center space-y-4">
          <h3 className="text-xl font-serif text-tea-text print:text-black">{product.givenName}</h3>
          <p className="text-tea-muted text-sm font-serif italic print:text-tea-text-dim">{product.productName} ({product.year})</p>
          
          <div className="flex justify-center p-4 bg-white rounded-xl mx-auto w-fit border border-tea-border/50 shadow-sm">
            <QRCodeCanvas value={qrValue} size={200} />
          </div>
          
          <div className="pt-4 text-[10px] text-tea-muted font-mono uppercase tracking-[0.2em] print:text-black">
            ID: {product.id}
          </div>
