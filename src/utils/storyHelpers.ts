...
export const parsePagesFromStory = (story: Story): PageState[] => {
  ...
  return story.content.map(block => {
    ...
    if (variantMatch) {
      if (Object.values(LayoutVariant).includes(variantMatch[1] as LayoutVariant)) {
        variant = variantMatch[1] as LayoutVariant;
      }
      rawContent = variantMatch[2] || '';
    }

    let textColor: 'light' | 'dark' = 'light';
    if (rawContent.startsWith('$$dark$$')) {
      textColor = 'dark';
      rawContent = rawContent.substring(8);
    }

    const parts = rawContent.split('|');
    const isUrl = (s: string) =>
      s && (s.match(/^https?:\/\//) || s.startsWith('data:image') || s.includes('picsum'));
