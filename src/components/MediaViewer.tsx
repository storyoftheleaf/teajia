...
const DesktopSidebar: React.FC<{ story: Story; onBack: () => void; onShare?: () => void; isSaved?: boolean; onToggleSave?: () => void }> = ({ story, onBack, onShare, isSaved, onToggleSave }) => (
  <aside className="w-24 xl:w-32 h-full bg-tea-moss text-tea-text flex flex-col items-center py-10 relative shadow-[4px_0_15px_rgba(0,0,0,0.5)] z-30 shrink-0">
    ...
    <div className="flex-1 flex items-center justify-center min-h-0 overflow-hidden">
      <h1 className="writing-vertical text-2xl xl:text-4xl font-serif tracking-caps uppercase text-tea-text whitespace-nowrap leading-normal truncate-vertical py-4">
        {story.title}
      </h1>
    </div>
...
const MobileHeader: React.FC<{ title: string; onBack: () => void; transparent?: boolean }> = ({ title, onBack, transparent }) => (
  <header className={`absolute top-0 left-0 w-full z-50 flex items-center justify-between px-4 h-16 transition-all duration-300 ${transparent ? 'bg-gradient-to-b from-tea-bg/80 to-transparent' : 'bg-tea-moss shadow-lg'}`}>
    ...
    {!transparent && (
       <h1 className="text-xs uppercase tracking-display font-serif text-tea-text opacity-90 truncate max-w-[200px]">
         {title}
       </h1>
    )}
      ...
      <div className="fixed inset-0 sidebar-inset bg-black z-modal">
        ...
        {!isPlaying && (
            ...
            <div className="flex items-center space-x-2 mb-3 opacity-90">
                <span className="px-2 py-0.5 border border-tea-border text-ui-9 uppercase tracking-caps rounded-full">
                    {story.type}
                </span>
            </div>
    ...
    <div className="fixed inset-0 sidebar-inset z-modal bg-tea-bg flex">
      ...
      <main className="flex-1 relative flex items-center justify-center p-12" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.04'/%3E%3C/svg%3E")` }}>
        <div className="flex flex-row h-full max-h-[80vh] w-full max-w-5xl items-center justify-center gap-12">
            ...
            {/* Info Panel */}
            <div className="max-w-sm text-tea-text flex flex-col justify-center h-full">
                <div className="w-8 h-[1px] bg-tea-gold mb-6"></div>
                <div className="flex items-center gap-3 text-xs tracking-display uppercase text-tea-text/70 mb-2">
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 sidebar-inset bg-tea-bg z-modal flex flex-col overflow-y-auto">
         ...
         {/* Content */}
         <div className="p-6 pb-[calc(44px+env(safe-area-inset-bottom,0px)+24px)] lg:pb-6 text-tea-text flex-1">
            <div className="flex justify-between items-start mb-2">
                <span className="text-ui-10 tracking-caps uppercase opacity-70 block">{story.type} • {story.durationOrTime}</span>
  ...
  return (
    <div className="fixed inset-0 sidebar-inset z-modal bg-tea-bg flex">
       ...
       <main className="flex-1 flex flex-col items-center overflow-y-auto">
          ...
          {/* Info Section */}
          <div className="w-full max-w-4xl p-12 text-tea-text">
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 sidebar-inset bg-tea-bg z-modal flex flex-col">
         ...
         {/* Controls */}
         <div className="bg-gradient-to-t from-tea-bg via-tea-bg to-transparent pt-12 px-8 pb-[calc(44px+env(safe-area-inset-bottom,0px)+12px)] lg:pb-12 z-20">
             <div className="flex justify-between items-end mb-8">
                <div className="flex-1 pr-4">
                    <h2 className="text-3xl font-serif text-tea-text mb-2 leading-tight">{story.title}</h2>
                    <p className="text-base text-tea-text/70 font-serif italic">{story.subtitle}</p>
                </div>
             </div>
             ...
             <div className="flex justify-between text-ui-10 text-tea-text/50 mb-10 font-mono tracking-caps">
                <span>04:20</span>
                <span>{story.durationOrTime}</span>
             </div>
         ...
         <div className="w-full max-w-6xl px-12 flex items-center justify-center gap-[clamp(2rem,5vw,5rem)] z-10">
            ...
            <div className="flex flex-col flex-1 max-w-lg">
               ...
               <div className="mb-4 group">
                    <div className="flex justify-between text-ui-10 text-tea-text/40 mb-2 font-mono tracking-caps">
                        <span>00:00</span>
                        <span>{story.durationOrTime}</span>
                    </div>
