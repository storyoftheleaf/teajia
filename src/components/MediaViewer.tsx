...
const DesktopSidebar: React.FC<{ story: Story; onBack: () => void; onShare?: () => void; isSaved?: boolean; onToggleSave?: () => void }> = ({ story, onBack, onShare, isSaved, onToggleSave }) => (
  <aside className="w-24 xl:w-32 h-full bg-tea-moss text-tea-bg flex flex-col items-center py-10 relative shadow-[4px_0_15px_rgba(0,0,0,0.5)] z-30 shrink-0">
    ...
    <button 
      ...
      title="Exit View"
    >
      <Icons.Back className="w-6 h-6 text-tea-bg/70 group-hover:text-tea-bg" />
    </button>

    {/* Vertical Title */}
    <div className="flex-1 flex items-center justify-center min-h-0 overflow-hidden">
      <h1 className="writing-vertical text-2xl xl:text-4xl font-serif tracking-[0.15em] uppercase text-tea-bg whitespace-nowrap leading-normal truncate-vertical py-4">
        {story.title}
      </h1>
    </div>
    ...
    <div className="mt-8 mb-0 flex flex-col space-y-6 items-center pt-4 border-t border-white/10 w-12">
         {onToggleSave && (
             <button onClick={onToggleSave} className={`p-2 rounded-full transition-colors ${isSaved ? 'text-tea-gold' : 'text-tea-bg/40 hover:text-tea-bg'}`} title="Collect">
               <Icons.Leaf filled={isSaved} className="w-5 h-5" />
             </button>
         )}
         <button onClick={onShare} className="p-2 rounded-full text-tea-bg/40 hover:text-tea-bg transition-colors" title="Share">
...
const MobileHeader: React.FC<{ title: string; onBack: () => void; transparent?: boolean }> = ({ title, onBack, transparent }) => (
  <header className={`absolute top-0 left-0 w-full z-50 flex items-center justify-between px-4 h-16 transition-all duration-300 ${transparent ? 'bg-gradient-to-b from-black/80 to-transparent' : 'bg-tea-moss shadow-lg'}`}>
    <button onClick={onBack} className="p-2 opacity-90 hover:opacity-100 text-tea-bg">
      <Icons.Back className="w-6 h-6" />
    </button>
    {!transparent && (
       <h1 className="text-xs uppercase tracking-[0.2em] font-serif text-tea-bg opacity-90 truncate max-w-[200px]">
         {title}
       </h1>
    )}
    ...
    return (
        <>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                ...
                >
                    <Icons.Play className="w-8 h-8 text-tea-bg ml-1 group-hover:text-white" />
                </button>
            </div>
        </>
    );
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 bg-black z-[60]">
        ...
        {!isPlaying && (
            <div className="absolute bottom-0 left-0 w-full p-6 pb-10 text-tea-bg pointer-events-none bg-gradient-to-t from-black/90 to-transparent">
    ...
    <div className="fixed inset-0 z-[60] bg-[#121212] flex">
      ...
      <main className="flex-1 relative flex items-center justify-center bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')] p-12">
        <div className="flex flex-row h-full max-h-[80vh] w-full max-w-5xl items-center justify-center gap-12">
            ...
            {/* Info Panel */}
            <div className="max-w-sm text-tea-bg flex flex-col justify-center h-full">
                <div className="w-8 h-[1px] bg-tea-gold mb-6"></div>
                <div className="flex items-center gap-3 text-xs tracking-[0.2em] uppercase text-tea-bg/70 mb-2">
                    <span>{story.origin}</span>
                    <span>•</span>
                    <span>{story.durationOrTime}</span>
                </div>
                <h2 className="text-4xl font-serif mb-2 leading-tight">{story.title}</h2>
                <p className="text-lg font-serif italic text-tea-bg/70 mb-8">{story.subtitle}</p>
                <p className="text-base leading-relaxed text-tea-bg/90 font-light">{story.description}</p>
            </div>
        </div>
      </main>
    </div>
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 bg-tea-bg z-[60] flex flex-col overflow-y-auto">
         ...
         {/* Content */}
         <div className="p-6 text-tea-bg flex-1">
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[#050505] flex">
       ...
       <main className="flex-1 flex flex-col items-center overflow-y-auto">
          ...
          {/* Info Section */}
          <div className="w-full max-w-4xl p-12 text-tea-bg">
             <div className="flex justify-between items-start mb-8">
                <div>
                   <h2 className="text-5xl font-serif mb-2">{story.title}</h2>
                   <p className="text-xl font-serif italic text-tea-bg/60">{story.subtitle}</p>
                </div>
                <div className="flex items-center space-x-6 text-tea-bg/70">
                   <button onClick={onToggleSave} className={`hover:text-tea-bg transition-colors ${isSaved ? 'text-tea-gold' : ''}`}>
                      <Icons.Leaf filled={isSaved} className="w-5 h-5" />
                   </button>
                   <button onClick={onShare} className="hover:text-tea-bg transition-colors">
                      <Icons.Share className="w-5 h-5" />
                   </button>
                </div>
             </div>
         ...
         <div className="bg-gradient-to-t from-tea-bg via-tea-bg to-transparent pt-12 px-8 pb-12 z-20">
             <div className="flex justify-between items-end mb-8">
                <div className="flex-1 pr-4">
                    <h2 className="text-3xl font-serif text-tea-bg mb-2 leading-tight">{story.title}</h2>
                    <p className="text-base text-tea-bg/70 font-serif italic">{story.subtitle}</p>
                </div>
             </div>
             ...
             <div className="flex justify-between text-[10px] text-tea-bg/50 mb-10 font-mono tracking-widest">
                <span>04:20</span>
                <span>{story.durationOrTime}</span>
             </div>
             ...
             <div className="flex items-center justify-between px-4">
                <button onClick={onToggleSave} className={`text-tea-bg/40 hover:text-tea-bg transition-colors ${isSaved ? 'text-tea-gold' : ''}`}>
                     <Icons.Leaf filled={isSaved} className="w-6 h-6" />
                </button>

                <div className="flex items-center space-x-8">
                   <Icons.Back className="w-8 h-8 text-tea-bg/50 rotate-180 hover:text-tea-bg transition-colors cursor-pointer" /> {/* Prev */}
                   ...
                   <Icons.Next className="w-8 h-8 text-tea-bg/50 hover:text-tea-bg transition-colors cursor-pointer" />
                </div>

                <button onClick={onShare} className="text-tea-bg/40 hover:text-tea-bg transition-colors">
         ...
         <div className="w-full max-w-6xl px-12 flex items-center justify-center gap-20 z-10">
            ...
            <div className="flex flex-col flex-1 max-w-lg">
               <div className="flex flex-col mb-10">
                    <span className="text-xs tracking-[0.3em] uppercase text-tea-bg/50 mb-4 border-l-2 border-tea-gold pl-3">Now Playing</span>
                    <h2 className="text-5xl xl:text-6xl font-serif text-tea-bg mb-4 leading-[1.1] tracking-tight">{story.title}</h2>
                    <p className="text-2xl font-serif italic text-tea-bg/60 font-light">{story.subtitle}</p>
               </div>
               ...
               <div className="mb-4 group">
                    <div className="flex justify-between text-[10px] text-tea-bg/40 mb-2 font-mono tracking-widest">
                        <span>00:00</span>
                        <span>{story.durationOrTime}</span>
                    </div>
               ...
               <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center space-x-8">
                      <button className="text-tea-bg/50 hover:text-tea-bg transition-colors"><Icons.Back className="w-6 h-6 rotate-180" /></button>
                      ...
                      <button className="text-tea-bg/50 hover:text-tea-bg transition-colors"><Icons.Next className="w-6 h-6" /></button>
                  </div>
