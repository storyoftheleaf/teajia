...
export const LearnCurriculum: React.FC<LearnCurriculumProps> = ({ onStoryClick, watchedStories }) => {
  ...
  const getDifficultyColor = (level: string): string => {
    switch(level) {
      ...
      case 'Advanced': return 'bg-tea-seal/20 text-tea-seal border border-tea-seal/40 shadow-sm';
      default: return 'bg-white/10 text-tea-paper/50';
    }
  };
  ...
  return (
    <div className="w-full animate-[fadeIn_0.5s_ease-out]">
      ...
      <div className="max-w-4xl mx-auto px-2 md:px-0 mb-6">
        <div className="inline-flex bg-tea-ink/5 dark:bg-white/5 rounded-full p-1">
          <button
            onClick={() => { setViewMode('courses'); setSelectedPath(null); }}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
              viewMode === 'courses'
                ? 'bg-tea-seal text-white'
                : 'text-tea-ink/60 dark:text-tea-paper/60 hover:text-tea-ink dark:hover:text-tea-paper'
            }`}
          >
            All Courses
          </button>
      ...
      <div className="max-w-4xl mx-auto px-2 md:px-0 mb-8">
        <div className="flex items-center justify-between">
          <p className="font-serif italic text-sm text-tea-ink/60 dark:text-tea-paper/60 leading-relaxed max-w-lg">
            {selectedPathData
          ...
          {allLessons.length > 0 && (
            <div className="flex items-center gap-4 text-xs text-tea-ink/50 dark:text-tea-paper/50 whitespace-nowrap ml-4">
      ...
      {viewMode === 'paths' && !selectedPath && (
        <div className="max-w-4xl mx-auto px-2 md:px-0 mb-12">
          <h2 className="text-lg font-serif text-tea-ink dark:text-tea-paper mb-6">Choose Your Path</h2>
            ...
            {LEARN_PATHS.map(path => (
              ...
              >
                <div className="p-6 flex flex-col items-center text-center gap-3">
                  <div className="text-tea-ink/40 dark:text-tea-paper/60">{getPathIcon(path.icon)}</div>
                  <h3 className="font-serif text-base text-tea-ink dark:text-tea-paper">{path.title}</h3>
                  <p className="text-xs text-tea-ink/50 dark:text-tea-paper/50 leading-relaxed">{path.description}</p>
                  <div className="text-[11px] uppercase tracking-wider text-tea-ink/40 dark:text-tea-paper/40 mt-2">
      ...
      {viewMode === 'paths' && selectedPath && (
        <div className="max-w-4xl mx-auto px-2 md:px-0 mb-6">
          <button
            ...
            className="flex items-center gap-2 text-tea-seal hover:text-tea-ink dark:hover:text-tea-paper transition-colors text-sm font-medium"
          >
          ...
          {selectedPathData && (
            <div className="mt-4 flex items-center gap-3">
              <div className="text-tea-ink/40 dark:text-tea-paper/60">{getPathIcon(selectedPathData.icon)}</div>
              <div>
                <h2 className="font-serif text-2xl text-tea-ink dark:text-tea-paper">{selectedPathData.title}</h2>
                <p className="text-sm text-tea-ink/60 dark:text-tea-paper/60 mt-1">{selectedPathData.description}</p>
              </div>
            </div>
          )}
        </div>
                 ...
                 <div key={module.id} className="group">
                     ...
                     >
                        ...
                        <div className="absolute top-0 left-0 w-full h-1 bg-white/5">
                            <div
                              className="h-full bg-tea-seal transition-all duration-1000 ease-out"
                        ...
                        <div className="p-5 md:p-7 flex items-start gap-5 md:gap-8">
                            ...
                            <div className="flex-1 pt-2">
                                <div className="flex justify-between items-start mb-2">
                                    <h3 className="font-serif text-2xl md:text-3xl text-tea-ink dark:text-tea-paper leading-tight group-hover:text-tea-seal transition-colors">
                                        {module.title}
                                    </h3>
                                    <div className="text-tea-ink/30 dark:text-tea-paper/30 transition-transform duration-500" style={{ transform: isExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                                        <Icons.ChevronDown className="w-6 h-6" />
                                    </div>
                                </div>
                                <p className="text-xs uppercase tracking-widest text-tea-ink/50 dark:text-tea-paper/50 mb-3 font-medium">
                                    {module.lessons.length} Lessons • {module.subtitle}
                                </p>
                                <div className={`grid transition-all duration-500 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                    <div className="overflow-hidden">
                                        <p className="font-serif italic text-tea-ink/60 dark:text-tea-paper/70 leading-relaxed max-w-lg">
                                            {module.description}
                                        </p>
                                    </div>
                                </div>
                     ...
                     <div className={`transition-all duration-700 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[2000px] opacity-100 mt-4' : 'max-h-0 opacity-0 mt-0'}`}>
                         <div className="pl-6 md:pl-24 pr-0 md:pr-8 flex flex-col relative">
                             {/* Vertical Path Line */}
                             <div className="absolute left-[1.6rem] md:left-[6.15rem] top-0 bottom-6 w-[1px] bg-tea-ink/10 dark:bg-white/10 z-0"></div>
                             ...
                             {module.lessons.map((lesson, lessonIndex) => {
                                 ...
                                 return (
                                     ...
                                     >
                                         {/* Status Node */}
                                         <div className={`w-3 h-3 rounded-full border z-10 shrink-0 transition-all duration-300 ${isWatched ? 'bg-tea-seal border-tea-seal' : 'border-tea-ink/40 dark:border-tea-paper/40 bg-white dark:bg-tea-ink group-hover/lesson:border-tea-ink dark:group-hover/lesson:border-tea-paper'}`}></div>

                                         {/* Lesson Card */}
                                         <div className={`flex-1 bg-tea-ink/5 dark:bg-white/5 hover:bg-tea-ink/10 dark:hover:bg-white/10 border border-tea-ink/5 dark:border-white/5 rounded-[1px] p-4 flex items-center justify-between transition-all duration-300 group-hover/lesson:-translate-y-0.5 group-hover/lesson:shadow-md ${!isLast ? 'border-b border-b-tea-ink/5 dark:border-b-white/5' : ''}`}>
                                             <div className="flex items-center gap-4">
                                                 {/* Type Icon Box */}
                                                 <div className="w-11 h-11 bg-tea-ink/10 dark:bg-white/10 rounded-[1px] flex items-center justify-center text-tea-ink/70 dark:text-tea-paper/70 shrink-0">
                                                     {getIconForType(lesson.type)}
                                                 </div>

                                                 <div className="flex flex-col gap-1.5">
                                                     {/* Primary: Title */}
                                                     <h4 className={`font-serif text-lg leading-tight transition-colors ${isWatched ? 'text-tea-ink/60 dark:text-tea-paper/60' : 'text-tea-ink dark:text-tea-paper group-hover/lesson:text-tea-seal'}`}>
                                                         {lesson.title}
                                                     </h4>
                                                     {/* Secondary: Subtitle + Duration */}
                                                     <div className="flex items-center gap-2">
                                                         <span className="text-[11px] uppercase tracking-wider text-tea-ink/60 dark:text-tea-paper/60">{lesson.subtitle}</span>
                                                         <span className="w-1 h-1 rounded-full bg-tea-ink/30 dark:bg-tea-paper/30"></span>
                                                         <span className="font-mono text-[11px] text-tea-ink/50 dark:text-tea-paper/50">{lesson.durationOrTime}</span>
                                                     </div>
                                                     {/* Tertiary: Badges */}
                                             ...
                                             <div className="opacity-0 group-hover/lesson:opacity-100 transition-all -translate-x-2 group-hover/lesson:translate-x-0 duration-300">
                                                 <Icons.Next className="w-5 h-5 text-tea-ink/40 dark:text-tea-paper/40" />
                                             </div>
                                         </div>
                                     </div>
                                 );
