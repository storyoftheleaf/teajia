                ...
                return (
                    <div className={`${paperBase} flex flex-col justify-between ${STD_PAD}`}>
                        ...
                        <div className="relative z-20 pb-8">
                            <EditableText value={storyTitle || ''} onChange={isEditable && onStoryUpdate ? (val) => onStoryUpdate('title', val) : undefined} className="text-8xl font-serif tracking-tight leading-[0.9] mb-6 text-tea-text" placeholder="Title" tag="h1" readOnly={readOnly} />
                            <EditableText value={storySubtitle || ''} onChange={isEditable && onStoryUpdate ? (val) => onStoryUpdate('subtitle', val) : undefined} className="text-4xl italic font-serif text-tea-text/80" placeholder="Subtitle" tag="p" readOnly={readOnly} />
                        </div>
                    </div>
                );
            case LayoutVariant.COVER_TYPOGRAPHIC:
                return (
                    <div className={`${paperBase} ${isDarkText ? 'bg-tea-bg text-tea-gold' : 'bg-tea-gold text-tea-text'} ${STD_PAD} flex flex-col justify-center`}>
                ...
                return (
                    <div className={`${paperBase} bg-black`}>
                        ...
                        <div className="absolute bottom-0 left-0 w-full p-12 pt-32 bg-gradient-to-t from-black/80 to-transparent z-10 pointer-events-none">
                            <div className={readOnly ? "" : "pointer-events-auto"}><EditableText value={content} onChange={isEditable ? updateContent : undefined} className="text-2xl uppercase tracking-[0.15em] text-tea-text/90" placeholder="Caption" tag="span" readOnly={readOnly} /></div>
                        </div>
                    </div>
                );
