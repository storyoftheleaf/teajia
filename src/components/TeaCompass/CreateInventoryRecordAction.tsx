...
export function CreateInventoryRecordAction({ entryId, productId, onOpenInventory }: Props) {
  ...
  if (linkedProductId) return (
    <button type="button" onClick={() => onOpenInventory(linkedProductId)} className="pill flex items-center gap-1 text-ui-12">
      <Check size={12} /> View in Inventory →
    </button>
  );
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <button type="button" onClick={() => void createInventoryRecord(entryId)} disabled={busy} className="pill flex items-center gap-1 text-ui-12 disabled:opacity-50">
