  ...
  return <BottomSheet open={open} onOpenChange={onOpenChange} title="Journey & visit" description="Optional context for where this was encountered" large>
    <div className="space-y-5 px-1">
      ...
      {editor?.kind === 'journey' && <div className="space-y-3 rounded-md border border-tea-border bg-tea-surface p-3">
        ...
        <div className="flex justify-between"><button className="tap-target text-ui-14 text-tea-text-sec" onClick={() => setEditor(null)}>Cancel</button><button disabled={pendingAction} className="min-h-11 rounded-md cta-solid px-4 text-ui-14 disabled:opacity-50" onClick={saveJourney}>{pendingAction ? 'Saving…' : 'Save journey'}</button></div>
      </div>}
      {editor?.kind === 'visit' && <div className="space-y-3 rounded-md border border-tea-border bg-tea-surface p-3">
        ...
        <div className="flex justify-between"><button className="tap-target text-ui-14 text-tea-text-sec" onClick={() => setEditor(null)}>Cancel</button><button disabled={pendingAction} className="min-h-11 rounded-md cta-solid px-4 text-ui-14 disabled:opacity-50" onClick={saveVisit}>{pendingAction ? 'Saving…' : 'Save visit'}</button></div>
      </div>}
      ...
      <div className="flex justify-between gap-3 px-3 pt-2"><button className="tap-target text-ui-14 text-tea-text-sec" onClick={() => { onApply(null, null); onOpenChange(false); }}>Clear context</button><button className="min-h-11 rounded-md cta-solid px-5 text-ui-14 font-medium" onClick={() => { onApply(selectedJourney, selectedVisit); onOpenChange(false); }}>Apply context</button></div>
    </div>
  </BottomSheet>;
};
