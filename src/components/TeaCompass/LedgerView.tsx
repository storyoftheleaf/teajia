    ...
    <div className="py-3 border-b border-tea-border/15 last:border-b-0">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          {item.chineseName && (
            item.productId ? (
              <button onClick={() => navigate(`/admin/inventory?panel=${encodeURIComponent(item.productId!)}`)} className="text-tea-text hover:text-tea-accent text-lg font-serif tracking-wide leading-[1.3] text-left transition-colors">
                {item.chineseName}
              </button>
            ) : (
