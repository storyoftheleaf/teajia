...
export const ImportPanel: React.FC<ImportPanelProps> = ({ initialDetail, onDetailChange, onFinalized, onClose, onNew, accountId }) => {
  ...
  const retryAction = useRef<null | (() => Promise<boolean | void>)>(null);
  ...
  const updateItem = async (item: CurateImportItem, updates: CurateImportItemUpdate, retryCurrentDraft?: () => Promise<boolean>) => {
    ...
    try { setOperationError(null); await action(); retryAction.current = null; return true; }
    catch (error) {
      retryAction.current = retryCurrentDraft ?? (async () => { await action(); return true; });
