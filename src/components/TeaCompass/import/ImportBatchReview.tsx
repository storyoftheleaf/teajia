...
export const ImportBatchReview: React.FC<ImportBatchReviewProps> = ({ detail, busyId, onAccept, onMerge, onAcceptAll, onDefer }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {detail.items.slice(0, visibleCount).map(item => <ImportItemRow key={item.id} item={item} busy={busyId === item.id} onAccept={() => onAccept(item)} onMerge={() => onMerge(item)} />)}
      </div>
