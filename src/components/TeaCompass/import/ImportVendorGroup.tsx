    ...
    <section data-testid="import-vendor-group" aria-labelledby={`vendor-${group.id}`} className="space-y-2 border-t border-tea-border pt-4 first:border-t-0">
      <div>
        <div className="flex min-h-8 items-start justify-between gap-3">
          <p className="pt-1 text-ui-10 uppercase tracking-[1.2px] text-tea-text-dim">Vendor · {group.items.length} {group.items.length === 1 ? 'tea' : 'teas'}{!group.vendorResolved ? ' · suggested' : ''}</p>
      ...
      {changing && (
        <div className="space-y-3 border-l-2 border-tea-border pl-3">
          <label className="block text-ui-11 text-tea-text-sec">Existing vendor
            <select aria-label={`Vendor for ${vendorName}`} value={group.resolved_vendor_customer_id || ''} disabled={Boolean(busyId)} onChange={event => { if (event.target.value) void onChangeVendor(group.id, event.target.value).then(() => setChanging(false)); }} className="mt-1 min-h-11 w-full rounded-md border border-tea-border bg-tea-elevated px-3 text-ui-16 text-tea-text outline-none focus:border-tea-gold lg:text-ui-13">
          ...
          <div className="flex justify-between gap-3">
            ...
            <button type="button" disabled={Boolean(busyId) || !newVendorName.trim()} onClick={() => void onCreateVendor(group.id, newVendorName.trim()).then(() => { setNewVendorName(''); setChanging(false); })} className="tap-target min-h-11 rounded-md bg-tea-gold px-4 text-ui-12 font-medium text-tea-bg disabled:opacity-50">Create vendor</button>
          </div>
        </div>
      )}
