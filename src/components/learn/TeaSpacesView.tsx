...
export const TeaSpacesView: React.FC<TeaSpacesViewProps> = ({ onBack, onNavigateToConsult }) => {
  ...
  return (
    <div className="w-full pb-32 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="mb-6">
        <h2 className="font-serif text-xl text-tea-ink dark:text-tea-paper mb-1">Tea Space Inspiration</h2>
        <p className="text-sm text-tea-ink/50 dark:text-tea-paper/50 font-serif italic">Ideas for creating your own ceremony environment</p>
      </div>
              ...
              <CardContainer variant="dark" className={`transition-all h-full ${expandedSpace === space.id ? 'shadow-md' : 'hover:-translate-y-0.5'}`}>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded-sm bg-tea-seal/8 dark:bg-tea-seal/15 text-tea-seal border border-tea-seal/15">
                ...
                <div className="space-y-2">
                  {space.tips.map((tip, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <div className="w-4 h-4 rounded-full bg-tea-seal/8 flex items-center justify-center shrink-0 mt-0.5">
      ...
      {onNavigateToConsult && (
        <button
          ...
          className="flex items-center justify-between w-full py-3 px-4 rounded-lg bg-tea-seal/5 dark:bg-tea-seal/8 hover:bg-tea-seal/8 dark:hover:bg-tea-seal/12 transition-colors group"
        >
