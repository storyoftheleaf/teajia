...
- `StaffView.tsx` — bundle-aware delegated tools where used.
- `TastingJournalView.tsx`, `MyCollection.tsx`, `CellarView.tsx` — personal tea surfaces.
- `workflows.ts` — link registries and operator-readiness definitions.
- `primitives.tsx` — shared identity, status, typography, and preview primitives.
- `types.ts` — panel-local view types.

## Current surface model

- Signed out: `ReaderView`.
- Signed in: `LaunchpadView`, with tiles filtered by role, memberships, and delegated capabilities.
...
- Panel uses `z-modal`; backdrop uses `z-drawer`.
