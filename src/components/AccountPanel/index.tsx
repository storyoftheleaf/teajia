...
export const AccountPanel: React.FC<AccountPanelProps> = ({ onClose, onNavigateToStory, initialView }) => {
      ...
      <div className="flex items-start justify-between px-4 py-3.5 bg-tea-surface">
        ...
        {showSwitchButton && (
          <button
            ...
            className="text-ui-11 uppercase tracking-caps text-tea-text-sec hover:text-tea-gold transition-colors shrink-0 ml-3 mt-0.5 py-2 -my-2"
          >
            Switch
          </button>
        )}
      ...
      {!hideActions && (
        <div className="flex border-t border-tea-border divide-x divide-tea-border">
          <button
            ...
            className="flex-1 py-3 text-ui-11 uppercase tracking-caps text-tea-text-sec hover:text-tea-gold hover:bg-tea-surface/50 transition-colors font-medium"
          >
            Shop
          </button>
          <button
          ...
          {isStaff && (
            <button
              ...
              className="flex-1 py-3 text-ui-11 uppercase tracking-caps text-tea-gold hover:bg-tea-surface/50 transition-colors font-medium"
            >
              Ops
            </button>
          )}
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bottom-nav lg:bottom-0 z-panel-backdrop bg-black/80 backdrop-blur-sm ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        style={{ transition: isVisible ? 'opacity 300ms ease' : 'opacity 150ms ease' }}
        onClick={onClose}
      />

      {/* Panel — sits above the mobile bottom nav so it stays tappable */}
      <div
        ref={focusTrapRef}
        className="fixed top-0 right-0 bottom-nav lg:bottom-0 w-full md:w-[400px] bg-tea-bg z-panel-modal shadow-2xl flex flex-col"
        style={{
      ...
      >
        ...
        <div className="relative flex items-center px-4 py-2.5 border-b border-tea-border bg-tea-surface">
          ...
          <h2 className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-[18px] font-serif text-tea-text tracking-tight truncate max-w-[60%] text-center">
            {headerTitle}
          </h2>
                        ...
                        >
                          ...
                          >
                            <div className="w-12 shrink-0 text-center pt-0.5">
                              <div className="text-ui-11 tracking-display text-tea-text-sec uppercase font-medium">{day}</div>
                          ...
                          {!isPast && !isFull && (
                            <div className="mt-3 ml-[calc(3rem+0.875rem)]">
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 px-4 py-2.5 text-ui-11 uppercase tracking-caps font-medium bg-tea-gold/10 text-tea-gold border border-tea-gold/30 rounded hover:bg-tea-gold/20 transition-colors min-h-[44px]"
