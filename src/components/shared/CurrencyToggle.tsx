    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-full right-0 mt-1 z-[101] bg-tea-ink border border-white/10 rounded-sm shadow-2xl overflow-hidden animate-[scaleIn_0.15s_ease-out] origin-top-right min-w-[140px]">
            {/* Currency options */}
            <div className="border-b border-white/5 px-3 py-2">
              <span className="text-[9px] uppercase tracking-widest text-tea-paper/40">Currency</span>
            </div>
            {DISPLAY_CURRENCIES.map((c) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs font-mono hover:bg-white/10 transition-colors flex items-center justify-between ${
                  currency === c ? 'text-tea-seal' : 'text-tea-paper/70'
                }`}
              >
                <span>{CURRENCY_SYMBOLS[c as CostCurrency]} {c}</span>
                {currency === c && <span className="text-tea-seal">✓</span>}
              </button>
            ))}

            {/* Unit options */}
            <div className="border-t border-white/5 border-b border-white/5 px-3 py-2">
              <span className="text-[9px] uppercase tracking-widest text-tea-paper/40">Weight</span>
            </div>
            {(['g', 'oz'] as const).map((u) => (
              <button
                key={u}
                ...
                className={`w-full text-left px-3 py-2 text-xs font-mono hover:bg-white/10 transition-colors flex items-center justify-between ${
                  unit === u ? 'text-tea-seal' : 'text-tea-paper/70'
                }`}
              >
                <span>{u === 'g' ? 'Grams (g)' : 'Ounces (oz)'}</span>
                {unit === u && <span className="text-tea-seal">✓</span>}
              </button>
            ))}
          </div>
        </>
