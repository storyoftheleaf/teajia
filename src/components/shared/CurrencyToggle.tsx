  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1.5 text-[10px] uppercase tracking-widest font-mono text-tea-bg/70 bg-white/5 border border-white/10 rounded-sm hover:bg-white/10 transition-colors"
      >
        ...
        <>
          ...
          <div className="absolute top-full right-0 mt-1 z-[101] bg-tea-elevated border border-tea-border rounded-sm shadow-2xl overflow-hidden animate-[scaleIn_0.15s_ease-out] origin-top-right min-w-[140px]">
            ...
            <div className="border-b border-white/5 px-3 py-2">
              <span className="text-[9px] uppercase tracking-widest text-tea-bg/40">Currency</span>
            </div>
            {DISPLAY_CURRENCIES.map((c) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs font-mono hover:bg-white/10 transition-colors flex items-center justify-between ${
                  currency === c ? 'text-tea-gold' : 'text-tea-bg/70'
                }`}
              >
            ...
            <div className="border-t border-white/5 border-b border-white/5 px-3 py-2">
              <span className="text-[9px] uppercase tracking-widest text-tea-bg/40">Weight</span>
            </div>
            {(['g', 'oz'] as const).map((u) => (
              <button
                key={u}
                ...
                className={`w-full text-left px-3 py-2 text-xs font-mono hover:bg-white/10 transition-colors flex items-center justify-between ${
                  unit === u ? 'text-tea-gold' : 'text-tea-bg/70'
                }`}
              >
