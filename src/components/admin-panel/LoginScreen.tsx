...
export const LoginScreen: React.FC<{ onLogin: () => void; onClose: () => void }> = ({ onLogin, onClose }) => {
    ...
    return (
        <div className="w-full h-full bg-tea-bg flex items-center justify-center p-6 md:pb-6">
            <button onClick={onClose} className="absolute top-6 left-6 p-2 text-tea-text-sec hover:text-tea-text" aria-label="Close login screen"><Icons.Close className="w-6 h-6" /></button>
                ...
                {!createdKey ? (
                    <>
                        <input
                            ...
                            placeholder="Passkey"
                            className="w-full bg-tea-surface border border-tea-border p-3 text-center text-tea-text tracking-[0.2em] outline-none focus-visible:ring-2 focus-visible:ring-tea-gold/50 focus-visible:ring-offset-1 focus-visible:ring-offset-tea-bg focus:border-tea-gold rounded-sm mb-4 placeholder:text-tea-text/30"
                        />
                        ...
                        <div className="border-t border-tea-border pt-6">
                            ...
                            <button
                                ...
                                className="text-tea-gold text-xs uppercase tracking-[0.15em] hover:text-tea-text transition-colors border border-tea-border px-4 py-2 rounded-sm hover:bg-tea-gold/10"
                            >
                                Generate New Key
                            </button>
                        </div>
