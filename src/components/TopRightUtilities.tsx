    ...
    <div className={`lg:hidden fixed top-4 right-4 z-50 flex items-center gap-3 transition-opacity duration-300 ${hidden ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      >
        ...
        {cartItemCount > 0 && (
          <div
            key={cartItemCount}
            className="absolute -top-1 -right-1 w-5 h-5 bg-tea-gold text-tea-bg text-[10px] font-bold rounded-full flex items-center justify-center animate-[scaleIn_0.3s_ease-out] shadow-md"
          >
