...
export interface ContributorPullQuote {
  pull_quote: string;
  author_id: string;             // contributor slug who wrote the quote
  published_at: string;
  article_slug: string;
...
export interface DbArticle {
  ...
  // Contributor weave (added by migration 059):
  // subject_ids: contributor slugs this article is *about* (interviews, profiles).
  // pull_quote / pull_quote_subject: a single ~30-word line that surfaces on
  // the named subject's profile page.
  subject_ids?: string[];
  pull_quote?: string;
