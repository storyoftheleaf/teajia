        ...
        <header style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,360px),1fr))', alignItems: 'stretch', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.14)', minHeight: '90vh' }}>
          ...
          <div style={{ position: 'relative', order: 1, overflow: 'hidden', minHeight: '48vh', background: 'linear-gradient(155deg,var(--tj-read-empty-from) 0%,var(--tj-read-bg) 80%)' }}>
            ...
            <svg viewBox="0 0 400 400" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
              <g fill="none" stroke="rgb(var(--tj-read-gold-rgb) / 0.32)" strokeWidth="1.1">
                <ellipse cx="200" cy="244" rx="92" ry="20" />
                <path d="M108 244 C108 196, 132 168, 200 166 C268 168, 292 196, 292 244" />
                <path d="M126 200 C150 188, 250 188, 274 200" stroke="rgb(var(--tj-read-gold-rgb) / 0.2)" />
                <ellipse cx="200" cy="168" rx="68" ry="14" stroke="rgb(var(--tj-read-gold-rgb) / 0.22)" />
              </g>
        ...
        <section data-reveal style={{ padding: 'clamp(20px,4vw,40px) clamp(20px,5vw,56px) clamp(40px,6vw,72px)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,240px),1fr))', gap: 'clamp(14px,2.4vw,26px)', maxWidth: 1180, margin: '0 auto' }}>
            ...
            <figure style={{ margin: 0 }}>
              <div style={{ position: 'relative', aspectRatio: '4/5', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.2)', borderRadius: 3, overflow: 'hidden', background: 'linear-gradient(160deg,var(--tj-read-empty-from),var(--tj-read-plate-to))' }}>
                ...
                <svg viewBox="0 0 240 300" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
                  <g fill="none" stroke="rgba(150,180,180,0.3)" strokeWidth="1">
                    ...
                    <circle cx="120" cy="170" r="6" fill="rgb(var(--tj-read-gold-rgb) / 0.5)" />
                  </g>
                </svg>
            ...
            <figure style={{ margin: 0 }}>
              <div style={{ position: 'relative', aspectRatio: '4/5', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.2)', borderRadius: 3, overflow: 'hidden', background: 'radial-gradient(ellipse 60% 50% at 50% 64%, rgba(120,150,150,0.2), transparent 65%), #1a1610' }}>
            ...
            <figure style={{ margin: 0 }}>
              <div style={{ position: 'relative', aspectRatio: '4/5', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.2)', borderRadius: 3, overflow: 'hidden', background: 'linear-gradient(160deg,#2a1c12,var(--tj-read-plate-to)), radial-gradient(ellipse 70% 40% at 50% 80%, rgba(200,110,40,0.25), transparent 60%)' }}>
                <div aria-hidden="true" style={grainCss('0.8', 120)} />
        ...
        <section data-reveal style={{ maxWidth: 1040, margin: '0 auto', padding: 'clamp(30px,5vw,56px) 24px' }}>
          <div style={{ border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.18)', borderRadius: 4, background: 'linear-gradient(160deg,var(--tj-read-card-from),var(--tj-read-card-to))', padding: 'clamp(24px,4vw,44px)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,220px),1fr))', gap: 'clamp(28px,5vw,56px)', alignItems: 'center' }}>
              <div style={{ position: 'relative', aspectRatio: '1/1', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.16)', borderRadius: 3, overflow: 'hidden', background: 'radial-gradient(ellipse 70% 60% at 50% 38%, rgba(150,180,180,0.14), transparent 64%), linear-gradient(160deg,#20231f,var(--tj-read-bg))' }}>
                <svg viewBox="0 0 300 300" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
                  <g fill="none" stroke="rgba(150,180,180,0.4)" strokeWidth="1.2">
                    ...
                    <ellipse cx="150" cy="96" rx="58" ry="13" stroke="rgb(var(--tj-read-gold-rgb) / 0.4)" />
                    <path d="M104 250 C120 238, 180 238, 196 250" stroke="rgb(var(--tj-read-gold-rgb) / 0.3)" />
                  </g>
                </svg>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {[['Walls', 'Eggshell-thin, translucent'], ['Glaze', 'Qingbai, pooled pale blue'], ['In the hand', 'Light, balanced, warm']].map(([k, v]) => (
                  <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, padding: '13px 0', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.1)' }}>
