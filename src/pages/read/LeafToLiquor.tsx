...
const Manuscript: React.FC = () => (
  <article style={{ position: 'relative', zIndex: 1 }}>
    ...
    <header style={{ position: 'relative', minHeight: '92vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 'clamp(40px,8vw,90px) 24px', overflow: 'hidden' }}>
      <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 50% 18%, rgb(var(--tj-read-gold-rgb) / 0.13) 0%, transparent 62%), radial-gradient(ellipse 60% 50% at 50% 100%, rgb(var(--tj-read-gold-rgb) / 0.06) 0%, transparent 60%)' }} />
      <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-54%)', fontFamily: F.cn, fontWeight: 200, fontSize: 'min(58vw,560px)', lineHeight: 1, color: 'rgb(var(--tj-read-gold-rgb) / 0.05)', pointerEvents: 'none', userSelect: 'none' }}>茶</div>
    ...
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: '0 24px' }}>
      ...
      <section style={{ maxWidth: 680, margin: '0 auto', padding: 'clamp(40px,6vw,72px) 0 0' }}>
        ...
        <aside style={{ float: 'right', width: 'clamp(150px,38%,220px)', margin: '6px 0 14px 30px', paddingLeft: 16, borderLeft: '1px solid rgb(var(--tj-read-gold-rgb) / 0.28)' }}>
      ...
      <figure data-reveal style={{ maxWidth: 760, margin: '0 auto', padding: '0 0 clamp(20px,4vw,40px)' }}>
        <div style={{ position: 'relative', aspectRatio: '16/10', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.2)', borderRadius: 3, overflow: 'hidden', background: 'linear-gradient(150deg,var(--tj-read-plate-from) 0%,var(--tj-read-plate-to) 100%)' }}>
          ...
          <svg viewBox="0 0 760 475" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
            <g fill="none" stroke="rgb(var(--tj-read-gold-rgb) / 0.5)" strokeWidth="1.1">
              <path d="M150 250 C 250 140, 510 140, 610 250 C 510 360, 250 360, 150 250 Z" />
              <path d="M150 250 C 250 200, 510 200, 610 250" stroke="rgb(var(--tj-read-gold-rgb) / 0.32)" />
              <path d="M150 250 C 250 300, 510 300, 610 250" stroke="rgb(var(--tj-read-gold-rgb) / 0.32)" />
              <line x1="380" y1="158" x2="380" y2="342" stroke="rgb(var(--tj-read-gold-rgb) / 0.4)" />
            </g>
            <g stroke="rgb(var(--tj-read-gold-rgb) / 0.4)" strokeWidth="1"><line x1="300" y1="218" x2="300" y2="120" /><line x1="470" y1="290" x2="560" y2="380" /></g>
        ...
        <figcaption style={{ display: 'flex', gap: 14, alignItems: 'baseline', marginTop: 16, paddingTop: 12, borderTop: '1px solid rgb(var(--tj-read-gold-rgb) / 0.14)' }}>
      ...
      <section style={{ maxWidth: 680, margin: '0 auto', padding: 'clamp(20px,4vw,40px) 0 clamp(40px,6vw,72px)' }}>
        ...
        <div data-reveal style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '0 22px' }}>
          <div style={{ gridColumn: '1/-1', height: 1, background: 'rgb(var(--tj-read-gold-rgb) / 0.16)' }} />
          {SIX.map((s, i) => (
            <React.Fragment key={s.name}>
              ...
              <div style={{ gridColumn: '1/-1', height: 1, background: i === SIX.length - 1 ? 'rgb(var(--tj-read-gold-rgb) / 0.16)' : 'rgb(var(--tj-read-gold-rgb) / 0.10)' }} />
            </React.Fragment>
          ))}
        </div>
      </section>
...
const Gallery: React.FC = () => (
  <article style={{ position: 'relative', zIndex: 1 }}>
    {/* COVER, asymmetric */}
    <header style={{ position: 'relative', minHeight: '96vh', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,400px),1fr))', alignItems: 'stretch', overflow: 'hidden', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.14)' }}>
      <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 'clamp(28px,5vw,72px)', gap: 0 }}>
        <div style={{ fontFamily: F.mono, fontSize: 'clamp(60px,9vw,130px)', fontWeight: 400, lineHeight: 0.9, color: 'rgb(var(--tj-read-gold-rgb) / 0.34)', letterSpacing: '-0.02em' }}>N°01</div>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(155deg,#2a2117 0%,var(--tj-read-bg) 75%)', minHeight: '46vh' }}>
        <div aria-hidden="true" style={{ ...grainCss('0.85', 160), opacity: 0.08 }} />
        <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '54%', transform: 'translate(-50%,-50%)', width: '74%', aspectRatio: '3/4', borderRadius: '50% 50% 48% 48%/60% 60% 40% 40%', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.4)', background: 'radial-gradient(ellipse 70% 60% at 50% 30%, rgb(var(--tj-read-gold-rgb) / 0.12), transparent 70%)' }} />
        <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '54%', transform: 'translate(-50%,-50%)', width: '50%', aspectRatio: '3/4', borderRadius: '50% 50% 48% 48%/60% 60% 40% 40%', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.26)' }} />
        <div aria-hidden="true" style={{ position: 'absolute', top: '50%', left: '54%', transform: 'translate(-50%,-50%)', width: 1, height: '62%', background: 'linear-gradient(rgb(var(--tj-read-gold-rgb) / 0.4),transparent)' }} />
    ...
    <figure data-reveal style={{ margin: 0, padding: 'clamp(20px,4vw,48px) 0' }}>
      <div style={{ position: 'relative', height: 'clamp(380px,68vh,720px)', overflow: 'hidden', background: 'linear-gradient(160deg,var(--tj-read-plate-from) 0%,var(--tj-read-plate-to) 90%)', borderTop: '1px solid rgb(var(--tj-read-gold-rgb) / 0.16)', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.16)' }}>
        <div aria-hidden="true" style={{ ...grainCss('0.8', 150), opacity: 0.07 }} />
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(100deg, transparent 0 46px, rgb(var(--tj-read-gold-rgb) / 0.05) 46px 47px)' }} />
        <div aria-hidden="true" style={{ position: 'absolute', left: '50%', top: '48%', transform: 'translate(-50%,-50%)', display: 'flex', gap: 'clamp(14px,3vw,42px)' }}>
          <div style={{ width: 'clamp(34px,5vw,72px)', height: 'clamp(200px,40vh,420px)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.3)', borderRadius: 60, background: 'linear-gradient(180deg, rgb(var(--tj-read-gold-rgb) / 0.08), transparent)' }} />
          <div style={{ width: 'clamp(34px,5vw,72px)', height: 'clamp(200px,40vh,420px)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.22)', borderRadius: 60 }} />
          <div style={{ width: 'clamp(34px,5vw,72px)', height: 'clamp(200px,40vh,420px)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.3)', borderRadius: 60, background: 'linear-gradient(180deg, rgb(var(--tj-read-gold-rgb) / 0.06), transparent)' }} />
        </div>
    ...
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,300px),1fr))', gap: 'clamp(28px,5vw,80px)', alignItems: 'center', padding: 'clamp(50px,8vw,110px) clamp(24px,6vw,96px)' }}>
      <div data-reveal>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 26 }}>
          ...
          <span style={{ flex: 1, height: 1, background: 'rgb(var(--tj-read-gold-rgb) / 0.2)' }} />
      ...
      <figure data-reveal style={{ margin: 0 }}>
        <div style={{ position: 'relative', aspectRatio: '4/5', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.22)', borderRadius: 3, overflow: 'hidden', background: 'radial-gradient(ellipse 70% 50% at 50% 22%, rgb(var(--tj-read-gold-rgb) / 0.1), transparent 65%), #1b1610' }}>
          ...
          <svg viewBox="0 0 320 400" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
            <g fill="none" stroke="rgb(var(--tj-read-gold-rgb) / 0.42)" strokeWidth="1.2"><path d="M160 70 C 110 150, 110 270, 160 330 C 210 270, 210 150, 160 70 Z" /><line x1="160" y1="90" x2="160" y2="320" /><path d="M160 150 C 140 160, 130 172, 124 190" /><path d="M160 190 C 182 200, 192 214, 198 232" /><path d="M160 230 C 140 240, 132 252, 126 270" /></g>
            <circle cx="160" cy="60" r="4" fill={C.gold} />
          </svg>
          <div style={{ position: 'absolute', bottom: 16, left: 18, fontFamily: F.cn, fontSize: 20, color: 'rgb(var(--tj-read-taupe-rgb) / 0.8)' }}>一芽二叶</div>
        </div>
    ...
    <section data-reveal style={{ padding: 'clamp(70px,11vw,150px) clamp(24px,6vw,96px)', textAlign: 'center', background: 'radial-gradient(ellipse 70% 80% at 50% 50%, rgb(var(--tj-read-gold-rgb) / 0.06), transparent 70%)', borderTop: '1px solid rgb(var(--tj-read-gold-rgb) / 0.1)', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.1)' }}>
    ...
    {/* CLOSING */}
    <section data-reveal style={{ padding: 'clamp(60px,9vw,120px) clamp(24px,6vw,96px) clamp(80px,11vw,150px)', textAlign: 'center', borderTop: '1px solid rgb(var(--tj-read-gold-rgb) / 0.12)' }}>
...
const figGesture = (svg: React.ReactNode, name: string, blurb: string, strong = false) => (
  <div style={{ flex: 1, minWidth: 130, textAlign: 'center' }}>
    <div style={{ width: 58, height: 58, margin: '0 auto 16px', borderRadius: '50%', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `radial-gradient(circle at 50% 35%, rgb(var(--tj-read-gold-rgb) / ${strong ? 0.18 : 0.12}), transparent 70%)` }}>{svg}</div>
...
const Folio: React.FC = () => (
  <article style={{ position: 'relative', zIndex: 1, maxWidth: 1140, margin: '0 auto', padding: '0 clamp(20px,5vw,56px)' }}>
    {/* TITLE PAGE */}
    <header style={{ padding: 'clamp(60px,10vw,128px) 0 clamp(40px,5vw,64px)', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.2)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: '18px 28px', borderTop: '1px solid rgb(var(--tj-read-gold-rgb) / 0.14)', paddingTop: 22 }}>
    ...
    {/* FIG 1: process flow */}
    <figure data-reveal style={{ margin: 'clamp(44px,6vw,72px) 0', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.18)', borderRadius: 3, background: 'linear-gradient(160deg,var(--tj-read-card-from),var(--tj-read-card-to))', overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '16px clamp(18px,3vw,28px)', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.12)' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, padding: 'clamp(28px,4vw,48px) clamp(18px,3vw,28px)' }}>
        {figGesture(<svg width="26" height="26" viewBox="0 0 26 26" fill="none"><path d="M13 3 C8 9, 8 18, 13 23 C18 18, 18 9, 13 3Z M13 6v15" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.1" /></svg>, 'Pluck', 'Two leaves & a bud, gathered by hand.')}
        {figGesture(<svg width="26" height="26" viewBox="0 0 26 26" fill="none"><path d="M4 17h18M6 17c0-5 5-8 7-8s7 3 7 8" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.1" /><path d="M10 6v3M16 6v3" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.1" strokeLinecap="round" /></svg>, 'Wither', 'Moisture leaves; aroma wakes.')}
        {figGesture(<svg width="26" height="26" viewBox="0 0 26 26" fill="none"><circle cx="13" cy="13" r="9" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.1" /><path d="M13 13 L13 5 A8 8 0 0 1 20 16 Z" fill="rgb(var(--tj-read-gold-rgb) / 0.35)" /></svg>, 'Oxidise', 'The turning, halted, or let run.', true)}
        {figGesture(<svg width="26" height="26" viewBox="0 0 26 26" fill="none"><path d="M5 20c4-2 12-2 16 0M7 15c3-9 9-9 12 0" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.1" strokeLinecap="round" /></svg>, 'Dry', 'Fixed in time, ready to keep.')}
      </div>
    </figure>

    {/* FIG 2: oxidation chart */}
    <figure data-reveal style={{ margin: 'clamp(44px,6vw,72px) 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', paddingBottom: 16, borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.14)', marginBottom: 'clamp(36px,5vw,56px)' }}>
      ...
      <div style={{ position: 'relative', height: 118 }}>
        ...
        <div style={{ position: 'absolute', left: '1%', top: 78 }}><div style={{ width: 1, height: 18, background: 'rgb(var(--tj-read-gold-rgb) / 0.5)', marginTop: -12 }} /><div style={{ position: 'absolute', bottom: 34, left: 0, fontFamily: F.display, fontStyle: 'italic', fontSize: 15, color: C.ink, whiteSpace: 'nowrap' }}>Green</div><div style={{ position: 'absolute', top: 24, left: 0, fontFamily: F.mono, fontSize: 10, color: C.gold }}>0%</div></div>
        <div style={{ position: 'absolute', left: '11%', top: 78 }}><div style={{ width: 1, height: 14, background: 'rgb(var(--tj-read-gold-rgb) / 0.4)', marginTop: -9 }} /><div style={{ position: 'absolute', bottom: 18, left: 0, transform: 'translateX(-50%)', fontFamily: F.display, fontStyle: 'italic', fontSize: 13, color: C.taupe, whiteSpace: 'nowrap' }}>White</div></div>
        <div style={{ position: 'absolute', left: '50%', top: 78 }}><div style={{ width: 1, height: 18, background: 'rgb(var(--tj-read-gold-rgb) / 0.5)', marginTop: -12 }} /><div style={{ position: 'absolute', bottom: 34, left: 0, transform: 'translateX(-50%)', fontFamily: F.display, fontStyle: 'italic', fontSize: 15, color: C.ink, whiteSpace: 'nowrap' }}>Oolong</div><div style={{ position: 'absolute', top: 24, left: 0, transform: 'translateX(-50%)', fontFamily: F.mono, fontSize: 10, color: C.gold }}>15–80%</div></div>
        <div style={{ position: 'absolute', left: '99%', top: 78 }}><div style={{ width: 1, height: 18, background: 'rgb(var(--tj-read-gold-rgb) / 0.5)', marginTop: -12 }} /><div style={{ position: 'absolute', bottom: 34, right: 0, fontFamily: F.display, fontStyle: 'italic', fontSize: 15, color: C.ink, whiteSpace: 'nowrap' }}>Red</div><div style={{ position: 'absolute', top: 24, right: 0, fontFamily: F.mono, fontSize: 10, color: C.gold }}>100%</div></div>
      </div>
    ...
    <section data-reveal style={{ margin: 'clamp(44px,6vw,72px) 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', paddingBottom: 16, borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.14)', marginBottom: 6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.7fr 2fr', gap: 0, fontFamily: F.body }}>
        ...
        {SIX.map((s, i) => (
          <React.Fragment key={s.name}>
            ...
            <div style={{ gridColumn: '1/-1', height: 1, background: i === SIX.length - 1 ? 'rgb(var(--tj-read-gold-rgb) / 0.18)' : 'rgb(var(--tj-read-gold-rgb) / 0.09)' }} />
          </React.Fragment>
        ))}
      </div>
    </section>
    ...
    {/* FIG 3: tasting panel */}
    <figure data-reveal style={{ margin: 'clamp(44px,6vw,72px) 0', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.18)', borderRadius: 3, background: 'linear-gradient(160deg,var(--tj-read-card-from),var(--tj-read-card-to))', padding: 'clamp(24px,4vw,40px)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', paddingBottom: 18, borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.12)', marginBottom: 28 }}>
    ...
    {/* CLOSING */}
    <section data-reveal style={{ padding: 'clamp(40px,6vw,64px) 0 clamp(80px,11vw,140px)', borderTop: '1px solid rgb(var(--tj-read-gold-rgb) / 0.2)', marginTop: 'clamp(20px,4vw,40px)' }}>
...
const FolioMeter: React.FC<{ label: string; value: string; pct: number }> = ({ label, value, pct }) => (
  <div>
    ...
    <div style={{ height: 3, background: 'rgb(var(--tj-read-gold-rgb) / 0.12)', borderRadius: 2 }}><div style={{ height: '100%', width: `${pct}%`, background: C.gold, borderRadius: 2 }} /></div>
  </div>
);
...
const Thread: React.FC = () => {
  ...
  const node = (numeral: string) => (
    <div style={{ position: 'absolute', left: 'clamp(28px,7vw,46px)', top: 22, transform: 'translate(-50%,-50%)', width: 42, height: 42, borderRadius: '50%', background: 'var(--tj-read-card-to)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: F.cn, fontSize: 18, color: C.gold }}>{numeral}</div>
  );
  ...
  return (
    <article style={{ position: 'relative', zIndex: 1 }}>
      ...
      <header style={{ position: 'relative', minHeight: '88vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 'clamp(40px,8vw,90px) 24px 0', overflow: 'hidden' }}>
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 70% 50% at 50% 30%, rgb(var(--tj-read-gold-rgb) / 0.1), transparent 62%)' }} />
        ...
        <div aria-hidden="true" style={{ position: 'relative', marginTop: 'clamp(36px,6vw,64px)', width: 1, height: 'clamp(70px,12vh,130px)', background: 'linear-gradient(var(--tj-gold,var(--tj-gold, var(--tj-read-gold-default))),transparent)' }} />
      </header>
      ...
      <div ref={trackRef} style={{ position: 'relative', maxWidth: 840, margin: '0 auto', padding: 'clamp(10px,3vw,30px) clamp(20px,5vw,40px) clamp(40px,7vw,80px) clamp(20px,5vw,30px)' }}>
        <div aria-hidden="true" style={{ position: 'absolute', left: 'clamp(28px,7vw,46px)', top: 0, bottom: 'clamp(40px,7vw,80px)', width: 1, background: 'rgb(var(--tj-read-gold-rgb) / 0.16)' }} />
        <div ref={fillRef} aria-hidden="true" style={{ position: 'absolute', left: 'clamp(28px,7vw,46px)', top: 0, width: 1, height: 0, background: 'linear-gradient(var(--tj-gold,var(--tj-gold, var(--tj-read-gold-default))),var(--tj-gold-lt,var(--tj-gold-lt, var(--tj-read-gold-lt-default))))', boxShadow: '0 0 9px rgb(var(--tj-read-gold-rgb) / 0.45)', transition: 'height 120ms linear' }} />

        {/* 一 Pluck */}
        <section data-reveal style={{ position: 'relative', padding: 'clamp(28px,5vw,48px) 0 clamp(40px,7vw,72px) clamp(58px,14vw,98px)' }}>
          <div style={{ position: 'absolute', left: 'clamp(28px,7vw,46px)', top: 'clamp(30px,5vw,52px)', transform: 'translate(-50%,-50%)', width: 42, height: 42, borderRadius: '50%', background: 'var(--tj-read-card-to)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: F.cn, fontSize: 18, color: C.gold }}>一</div>
        ...
        <section data-reveal style={stepPad}>
          ...
          <div style={{ position: 'relative', aspectRatio: '16/7', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.18)', borderRadius: 3, overflow: 'hidden', background: 'linear-gradient(150deg,var(--tj-read-plate-from),var(--tj-read-bg))' }}>
            <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(94deg, transparent 0 34px, rgb(var(--tj-read-gold-rgb) / 0.06) 34px 35px)' }} />
        ...
        {/* 三 Turn */}
        <section data-reveal style={stepPad}>
          <div style={{ position: 'absolute', left: 'clamp(28px,7vw,46px)', top: 22, transform: 'translate(-50%,-50%)', width: 42, height: 42, borderRadius: '50%', background: 'var(--tj-read-card-to)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.55)', boxShadow: '0 0 14px rgb(var(--tj-read-gold-rgb) / 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: F.cn, fontSize: 18, color: C.gold }}>三</div>
        ...
        <section data-reveal style={{ position: 'relative', padding: '0 0 clamp(28px,5vw,40px) clamp(58px,14vw,98px)' }}>
          ...
          <div style={{ position: 'relative', width: 'clamp(120px,30vw,150px)', aspectRatio: '1/1', margin: '0 auto' }}>
            ...
            <div aria-hidden="true" style={{ position: 'absolute', left: '50%', top: '18%', transform: 'translateX(-50%)', width: 2, height: '18%', background: 'linear-gradient(transparent,rgb(var(--tj-read-gold-rgb) / 0.5))', animation: 'tjFloat 3s ease-in-out infinite' }} />
          </div>
        </section>
      </div>
...
const Reverie: React.FC = () => (
  <article style={{ position: 'relative', zIndex: 1 }}>
    ...
    <section style={{ ...reverieSection, position: 'relative' }}>
      <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 60% 45% at 50% 42%, rgb(var(--tj-read-gold-rgb) / 0.1), transparent 65%)' }} />
      <div style={{ position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', bottom: 38, left: '50%', transform: 'translateX(-50%)', animation: 'tjFloat 3.4s ease-in-out infinite' }}><svg width="13" height="22" viewBox="0 0 13 22" fill="none"><path d="M6.5 1v18M1 13.5l5.5 5.5 5.5-5.5" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg></div>
    </section>

    {/* S2 one plant */}
    <section style={reverieSection}>
      <div style={{ fontFamily: F.cn, fontWeight: 200, fontSize: 'clamp(70px,16vw,200px)', lineHeight: 0.9, color: 'rgb(var(--tj-read-gold-rgb) / 0.16)', marginBottom: 10 }}>茶</div>
    ...
    {/* S4 wither */}
    <section style={{ ...reverieSection, background: 'radial-gradient(ellipse 70% 50% at 50% 60%, rgb(var(--tj-read-gold-rgb) / 0.05), transparent 70%)' }}>
    ...
    <section style={{ ...reverieSection, position: 'relative' }}>
      <div style={{ position: 'relative', width: 'clamp(180px,42vw,260px)', height: 'clamp(180px,42vw,260px)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 40 }}>
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.22)' }} />
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'radial-gradient(circle, rgb(var(--tj-read-gold-rgb) / 0.18), transparent 68%)', border: '1px solid rgb(var(--tj-read-gold-rgb) / 0.45)', animation: 'tjBreath 8s ease-in-out infinite' }} />
    ...
    {/* S7 six colours */}
    <section style={{ ...reverieSection, background: 'radial-gradient(ellipse 70% 50% at 50% 45%, rgb(var(--tj-read-gold-rgb) / 0.05), transparent 70%)' }}>
...
const LeafToLiquor: React.FC = () => {
  ...
  return (
    <ImmersiveRoot rootRef={rootRef}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '13px clamp(18px,4vw,40px)', background: 'rgb(var(--tj-read-bg-rgb) / 0.72)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', borderBottom: '1px solid rgb(var(--tj-read-gold-rgb) / 0.12)' }}>
        <a href="/read" style={{ display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none', color: 'inherit', minWidth: 0 }}>
          <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M9.5 3.5L5 7.5l4.5 4" stroke="var(--tj-gold, var(--tj-read-gold-default))" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" /></svg>
