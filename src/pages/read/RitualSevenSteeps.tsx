 ...
 * A gongfu cha scrollytelling piece: one handful of leaves, seven steeps.
 * Ported pixel-faithfully from the tea-article-redesign mockup.
 *
 * Bespoke effects ported from the design's <script> controller:
 *   1. Canvas steam animation (startSteam / rAF loop)
 *   2. Scroll-pinned cup: updatePour drives active steep detection + liquor-color morph
 *   3. Recap strip builder: buildRecap renders colour swatches dynamically
 *   4. Tea-palette prop: three datasets (oolong / puer / green) swap colours+temps+notes
 */
import React, { useEffect, useRef, useState } from 'react';
import { Helmet } from 'react-helmet-async';
import {
  C, F, ImmersiveRoot, ImmersiveNav, MoreFooter,
  useReveals, useImmersiveChrome, ACCENTS,
} from './immersive';

// ─── Tea datasets ─────────────────────────────────────────────────────────────
type TeaKey = 'oolong' | 'puer' | 'green';

const TEA_DATA: Record<TeaKey, {
  colors: string[];
  temps: number[];
  times: number[];
  notes: string[];
...
const CN_NUM = ['醒','一','二','三','四','五','六','七'];
const EN_LABEL = ['Rinse','First Infusion','Second Infusion','Third Infusion','Fourth Infusion','Fifth Infusion','Sixth Infusion','Seventh Infusion'];
...
// ─── Steep panel data ─────────────────────────────────────────────────────────
const STEEP_PANELS = [
  { steep: 0, kicker: '醒茶 · The Awakening',     title: 'First, we wake the leaf',  body: 'Boiling water is poured over the dry leaf and tipped away almost at once. It rinses, it warms the pot and cups, and — most of all — it coaxes tightly-rolled leaves to begin to open. We do not drink this water. We simply let the tea know it is time.' },
  { steep: 1, kicker: '第一泡 · First Infusion',   title: 'The first true cup',        body: 'Ten seconds, no more. The leaf has barely unclenched, and what it gives is all top-notes — the high, volatile aromatics that will never be this loud again. Light in the mouth, but the nose is already full. Drink it quickly; the second cup is already waiting.' },
  { steep: 2, kicker: '第二泡 · Second Infusion',  title: 'The body arrives',          body: 'Now the leaf is fully open and the liquor thickens. Texture fills the mouth where before there was only scent; sweetness gathers underneath. The tea stops being a fragrance and becomes a drink. This is where most teas first show you who they are.' },
  { steep: 3, kicker: '第三泡 · Third Infusion',   title: 'The peak',                  body: 'Aroma and body meet in the middle and hold hands. Brewers speak of the third and fourth steeps as the heart of a session — nothing is rushing in, nothing is fading yet. If you are sharing this tea with someone, this is the cup you give them.' },
  { steep: 5, kicker: '第五泡 · Fifth Infusion',   title: 'Softening',                 body: 'The edges round off. What was bright is now mellow; what was structured is now gentle. A good tea does not fall off a cliff here — it descends a long, easy staircase, giving honey and warm minerals where it once gave fire.' },
  { steep: 6, kicker: '第六泡 · Sixth Infusion',   title: 'Quiet sweetness',           body: 'We steep longer to draw out what remains. The tea is pale now, and unhurried, and quietly sweet — the kind of cup you drink without talking. The leaf has given almost everything it carried down from the mountain.' },
  { steep: 7, kicker: '第七泡 · Seventh Infusion', title: 'A gentle goodbye',          body: 'A minute or more in the water for one last, faint cup — clear, soft, almost a memory of tea. A fine leaf could be pushed further still; we choose to stop here, while the ending is sweet. To know when a tea is finished is its own small skill.' },
] as const;

// steep 4 has JSX body so it's handled inline in the render
const STEEP_4_BODY = (
  <>
    We add a few seconds to the steep now, asking the leaf for a little more. The front of the cup quietens, but the{' '}
    <em style={{ fontStyle: 'italic', color: '#ede4d4' }}>finish</em>{' '}
    opens out — that long after-taste the Chinese call huí gān, the "returning sweetness" that climbs back up the throat once the cup is empty.
  </>
);
...
const RitualSevenSteeps: React.FC = () => {
  ...
  const tea: TeaKey = 'oolong'; // default; matches design prop default

  useImmersiveChrome(accent);
  const rootRef = useReveals([]);

  // ── Refs for bespoke DOM effects ───────────────────────────────────────────
  ...
  const pourSectionRef = useRef<HTMLElement>(null);

  const steamIntensityRef = useRef(0);
  const activeRef         = useRef(-1);
  const steamRAFRef       = useRef<number>(0);
  const pourTickRef       = useRef(false);

  // Progress bar driven by a local state (ImmersiveNav reads it)
  ...
  const buildRecap = () => {
    ...
    el.innerHTML = d.colors.map((color, i) => `
      <div style="flex:1 1 64px; display:flex; flex-direction:column; align-items:center; gap:10px; min-width:60px;">
        <span style="width:clamp(34px,7vw,52px); height:clamp(34px,7vw,52px); border-radius:50%; background:${color}; box-shadow:0 0 0 1px rgba(168,135,77,0.3), inset 0 -6px 12px rgba(0,0,0,0.28), inset 0 5px 9px rgba(255,255,255,0.22); display:inline-block;"></span>
        <span style="font-family:'Noto Serif SC',serif; font-size:17px; color:#cdc0a8; line-height:1;">${CN_NUM[i]}</span>
  ...
  const setActive = (i: number) => {
    ...
    activeRef.current = i;
    const d = TEA_DATA[tea];
    if (steepNumRef.current)   steepNumRef.current.textContent   = CN_NUM[i];
    if (steepLabelRef.current) steepLabelRef.current.textContent = EN_LABEL[i];
    ...
    const section = pourSectionRef.current;
    if (!section) return;
    section.querySelectorAll<HTMLElement>('[data-steep]').forEach((p) => {
      const card = p.querySelector<HTMLElement>('[data-steep-card]');
      if (!card) return;
      const on = +(p.getAttribute('data-steep') || '-1') === i;
  ...
  const updatePour = () => {
    const section = pourSectionRef.current;
    if (!section) return;
    const panels = [...section.querySelectorAll<HTMLElement>('[data-steep]')];
    if (!panels.length) return;
    const d = TEA_DATA[tea];
    const vc = window.innerHeight * 0.5;
    const centers = panels.map(p => {
  ...
  // ── startSteam ─────────────────────────────────────────────────────────────
  const startSteam = (): (() => void) | undefined => {
    ...
    const handleResize = () => { resize(); updatePour(); };
    window.addEventListener('resize', handleResize);

    interface Part { x:number; y:number; vy:number; life:number; max:number; sway:number; swr:number; r:number; }
    const parts: Part[] = [];
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (pourTickRef.current) return;
      pourTickRef.current = true;
      requestAnimationFrame(() => { pourTickRef.current = false; updatePour(); });
    };
    window.addEventListener('scroll', onScroll, { passive: true });
    onScroll();

    const steamCleanup = startSteam();

    return () => {
      window.removeEventListener('scroll', onScroll);
      if (steamRAFRef.current) cancelAnimationFrame(steamRAFRef.current);
      if (steamCleanup) steamCleanup();
    };
      ...
      <article style={{ position: 'relative', zIndex: 1 }}>
        ...
        <header style={{
          ...
          <div style={{
            ...
            alignItems: 'center',
            gap: 12,
            animation: 'tjFloat 3.4s ease-in-out infinite',
          }}>
        ...
        <section
          ...
          style={{
            ...
            maxWidth: 1180,
            margin: '0 auto',
            padding: 'clamp(20px,4vw,40px) clamp(20px,5vw,40px) clamp(40px,7vw,80px)',
            display: 'grid',
        ...
        >
          ...
          <div style={{
            ...
            {/* Active steep readout */}
            <div style={{ marginTop: 'clamp(14px,3vw,28px)', textAlign: 'center', minHeight: 128 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14 }}>
                <span
                  ref={steepNumRef}
                  style={{ fontFamily: "'Noto Serif SC',serif", fontWeight: 200, fontSize: 46, lineHeight: 1, color: C.gold, transition: 'color 500ms' }}
                >醒</span>
                <div style={{ textAlign: 'left' }}>
                  <div ref={steepLabelRef} style={{ fontFamily: F.display, fontSize: 27, color: '#f3ead9', lineHeight: 1 }}>Rinse</div>
              ...
              <p
                ref={steepNoteRef}
                style={{ fontFamily: F.body, fontStyle: 'italic', fontSize: 'clamp(14px,1.7vw,16px)', lineHeight: 1.55, color: C.taupe, margin: '16px auto 0', maxWidth: 340 }}
              >A quick rinse to wake the leaf; this first water is poured away.</p>
            </div>
          </div>

          {/* Scrolling panels */}
          <div>
            {/* steep 0-3, 5-7 from array; steep 4 has JSX body */}
            {[0,1,2,3].map(idx => {
              const { steep, kicker, title, body } = STEEP_PANELS[idx];
              const isFirst = steep === 0;
              return (
                <div
                  key={steep}
                  data-steep={steep}
                  style={{ minHeight: '84vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '8px 0' }}
                >
                  <div
                    data-steep-card
                    style={{
                      borderLeft: isFirst ? 'calc(2px) solid var(--tj-gold,#a8874d)' : '2px solid rgba(168,135,77,0.18)',
                      padding: '6px 0 6px 24px',
                      opacity: isFirst ? 1 : 0.4,
                      transition: 'opacity 500ms, border-color 500ms',
                    }}
                  >
                    <div style={{ fontFamily: F.ui, fontSize: 10, fontWeight: 500, letterSpacing: '0.24em', textTransform: 'uppercase', color: C.dim, marginBottom: 14 }}>{kicker}</div>
                    <h2 style={{ fontFamily: F.display, fontWeight: 400, fontSize: 'clamp(28px,4.2vw,42px)', lineHeight: 1.08, color: '#f3ead9', margin: '0 0 18px' }}>{title}</h2>
                    <p style={{ fontFamily: F.body, fontSize: 'clamp(16px,2vw,18px)', lineHeight: 1.78, color: C.taupe, margin: 0 }}>{body}</p>
                  </div>
                </div>
              );
            })}

            {/* steep 4 — has inline JSX (huí gān em) */}
            <div
              data-steep={4}
              style={{ minHeight: '84vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '8px 0' }}
            >
              <div
                data-steep-card
                style={{ borderLeft: '2px solid rgba(168,135,77,0.18)', padding: '6px 0 6px 24px', opacity: 0.4, transition: 'opacity 500ms, border-color 500ms' }}
              >
                ...
                <h2 style={{ fontFamily: F.display, fontWeight: 400, fontSize: 'clamp(28px,4.2vw,42px)', lineHeight: 1.08, color: '#f3ead9', margin: '0 0 18px' }}>The finish lengthens</h2>
                <p style={{ fontFamily: F.body, fontSize: 'clamp(16px,2vw,18px)', lineHeight: 1.78, color: C.taupe, margin: 0 }}>{STEEP_4_BODY}</p>
              </div>
            </div>

            {[4,5,6].map(idx => {
              const { steep, kicker, title, body } = STEEP_PANELS[idx];
              const isLast = steep === 7;
              return (
                <div
                  key={steep}
                  data-steep={steep}
                  style={{ minHeight: isLast ? '88vh' : '84vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '8px 0' }}
                >
                  <div
                    data-steep-card
                    style={{ borderLeft: '2px solid rgba(168,135,77,0.18)', padding: '6px 0 6px 24px', opacity: 0.4, transition: 'opacity 500ms, border-color 500ms' }}
        ...
        <section data-reveal style={{ maxWidth: 1080, margin: '0 auto', padding: 'clamp(30px,5vw,56px) 24px' }}>
          ...
          <div
            ref={recapRef}
            style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: 'clamp(10px,2vw,18px)' }}
          />
