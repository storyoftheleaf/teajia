...
export const api = {
  ...
  people: {
    // Public — directory of published contributors.
    list: async () => {
      const res = await fetchWithTimeout(`${API_URL}/api/people`);
      return handleResponse(res);
    },
    // Public — single contributor profile, with woven content
    // (articles by, pull-quotes about, host account, products, seasonal stamp).
    getBySlug: async (slug: string) => {
