...
- [x] **Curate ingestion + purpose-based Inventory implementation** — Completed and verified locally on the feature branch 2026-07-12 (Tasks 1–18); **not yet deployed**. Curate remains one tap into the existing tea-first field sheet, accepts unordered fragments, groups optional Journey/Visit context, and adds reviewed Import for pasted text plus TXT/CSV/JSON extraction; images and PDFs are retained for manual review, not OCR-transcribed. Acquisitions create reviewed receipt proposals, while accepted Inventory receipts determine possession. Inventory separates Working/Sample/Personal purpose, named readiness omissions, receipt-derived Incoming, idempotent movements, safe same-identity transfers, and explicit storefront publication. Clean and legacy migration rehearsals, full automated verification, and both independent final reviews passed. This records implementation status only and makes no push or production-rollout claim. See `docs/superpowers/specs/2026-07-12-curate-inventory-ingestion-design.md` and the implementation plan for verification evidence.
