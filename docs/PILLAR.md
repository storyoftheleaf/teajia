...
Behind those four, the phone reaches them through the site panel (since 2026-09-22): the left end of the bottom bar is a three-line glyph that raises a panel the way the right end raises Your Table. Left door is the site (search, sessions, people, places, tea wisdom, cart, and the Manage rooms for anyone who has them), right door is you, the four words stay between them. Search left the bar to make the slot; it sits first in the panel and in every page header. The panel and the desktop Manage column read one list, `src/components/manageNav.ts`, so they cannot disagree about which rooms exist.
