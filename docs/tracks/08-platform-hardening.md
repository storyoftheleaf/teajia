...
Status: launch-critical tenancy coverage is implemented and locally verified; remaining items are post-launch hardening or optional scale work. External/manual evidence is tracked in [Launch Validation](../LAUNCH_VALIDATION.md).
...
- [x] **Tenancy-isolation test suite.** Release 1 added `worker/tests/tenancy-isolation.test.ts` with two-account fixtures, direct-resource/list denial, fulfillment/stock fencing, and separate platform-tier exceptions. Resource-specific tests add cross-account coverage for Compass, contributors, tasting-note curation, event/article association, inventory receipts, and verification journeys. This prerequisite for Track 2 is complete.
...
- [ ] **MCP: real-iPhone OAuth verification.** The transport fix is implemented: `/oauth/authorize` persists the request and redirects to path-based consent. Physical-device verification is tracked in [Launch Validation](../LAUNCH_VALIDATION.md); retain this technical item only for any evidence-driven follow-up if that check fails.
...
- [ ] **Durable offline write queue.** A partial localStorage queue now exists in `src/lib/offlineSync.ts` and is wired by `useOfflineSync`, but it currently queues only favorites and is not IndexedDB-backed general mutation durability. Extend or replace it only when China/rural use justifies the scope; preserve same-origin and service-worker assumptions. (day, when picked up)
- [ ] **Lossless transcription failure handling.** `handleTranscribe` still returns a bare failure when the Worker→Groq leg fails, and `useVoiceCapture` does not preserve the recorded blob for retry. Persist or return the raw audio reference and keep a client retry path so a network/provider failure never loses a sourcing recording. Actual mainland-network verification belongs in [Launch Validation](../LAUNCH_VALIDATION.md). (hours)
...
Physical-device OAuth, mainland-network, deployed-environment, and other human-only checks are maintained in [Launch Validation](../LAUNCH_VALIDATION.md). They are not duplicated as completion claims in this engineering backlog.
...
- The tenancy-isolation prerequisite for Track 2 is complete. Track 2 now owns the operator and production-state rollout.
- Offline mutation durability and lossless voice fallback inherit the shipped same-origin/service-worker policy; there is no separate China-reachability track to coordinate with.
