...
- [ ] **C2: rehost Unsplash/picsum images onto owned media.** `images.unsplash.com` (110 refs) and `picsum.photos` (62 refs) — 172 refs across 22 files, ~55 distinct base URLs — both blocked in China. Live hits: `src/content/photo-essays/*.ts` (5 files), `src/AboutPage.tsx`, `src/constants.ts`, `src/components/ResourcesPage.tsx`, `src/components/LearnLibrary.tsx`, `src/components/StoryModal.tsx`, `src/components/admin-panel/pageUtils.ts`, and 8 files under `src/data/*`. `public/_headers:7` still whitelists both hosts in `img-src`/`media-src` — safe to drop once rehosted.
