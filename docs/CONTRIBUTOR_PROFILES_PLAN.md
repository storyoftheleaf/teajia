...
> **Status:** Plan, partly built. Steps 1–3 of the build order are coded; steps 4 onward are pending.
> **Owner:** Adrian.
> **Last meaningful update:** 2026-05-04 (rebuild after a working-tree crash, recovered from paste + context).

## Why this exists

Articles, products, and events on Teajia are made by people. Until this feature, those people were flat strings — `articles.author_id` was a free-text slug, `Person` was a TypeScript-only interface, and there was no public route where a reader could meet a contributor. This plan replaces that with a real contributor record, a public `/people/:slug` page that reads as **a small ceremony, not a dossier**, and a network principle that lets a tea shop owner, an article subject, and a contributor be the same human without four different versions of them appearing across the site.

The page is the spine. Every other part of the platform — the catalogue, the magazine, the event calendar, the tasting community, the storefronts — connects through it.
...
10. **Hands on.** Teas this contributor had a hand in. *Not* a flat catalogue grid — grouped by the actual labor, with small italic Lora captions:
    ...
    Each group is a short row of products underneath. A reader learns the difference between sourcing, roasting, and curation just from reading the page. Section omitted if there are no attributions. When `face_of_account_id` is set, an additional caption may be added: *Carried by [Account Name]* — products the host's account stocks even if not directly attributed.
...
beginnings               TEXT                 -- the Origin paragraph (markdown)
now_text                 TEXT                 -- current preoccupation (markdown)
...
pouring_today_product_id TEXT                 -- soft reference to products(id)
pouring_today_note       TEXT
where_to_find_text       TEXT                 -- only rendered when face_of_account_id is set
...
```sql
ALTER TABLE articles ADD COLUMN subject_ids TEXT NOT NULL DEFAULT '[]'; -- JSON array of contributor slugs
...
```sql
ALTER TABLE products ADD COLUMN sourced_by TEXT; -- contributor slug
...
```sql
ALTER TABLE accounts ADD COLUMN host_contributor_id TEXT; -- contributor slug
```
...
Adrian writes these by hand. Twelve to twenty entries per active region. Never auto-generated.

### Migration from existing state

A migration reads the distinct values of `articles.author_id` and seeds a `contributors` row per slug, with `is_published = 0`. `display_name` is a placeholder (hyphens replaced with spaces). Adrian fills in the real fields through the admin editor.

Existing articles continue to render. The `formatAuthor()` helper in `ArticlePage.tsx` is replaced with a lookup that prefers a published `contributors.display_name` and falls back to the title-cased slug.

## Routes

- **`/people/:slug`** — public profile page (gated by `is_published`).
- **`/people`** — directory. v1 ships as a typeset alphabetical list. The "directory as map" upgrade is deferred.
...
- `face_of_account_id`: account picker. Setting this also writes `accounts.host_contributor_id` server-side.
...
1. **Migration + seed.** Create `contributors` table, seed from distinct `articles.author_id` values, all `is_published = 0`. Adds `face_of_account_id` and the integration columns on articles/products/accounts at the same time — cheap to add now, expensive to retrofit later.
2. **Public `/people/:slug` page.** Reads from DB. Renders Origin, Now, Inspirations, Words, Elsewhere, Closing. No portraits or pull-quotes yet. `formatAuthor()` swap so existing articles light up the link.
3. **Admin contributor list + editor panel.** Adrian can write Beginnings / Now / Inspirations / Closing for the contributors that matter most, publish them, and have the public page render. Includes the `face_of_account_id` field that mirrors to `accounts.host_contributor_id`.
