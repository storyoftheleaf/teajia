...
- [ ] **Curate ingestion + purpose-based Inventory rollout** — Tasks 1–17 were implemented and integrated on 2026-07-12; Task 18 full automated verification and final independent reviews remain pending before rollout completion. Curate remains one tap into the existing tea-first field sheet, accepts unordered fragments, groups optional Journey/Visit context, and adds a reviewed Import path for pasted lists plus retained image/PDF evidence. Uploaded evidence is not OCR-transcribed yet. Inventory separates Working/Sample/Personal purpose, named readiness omissions, open receipts/Incoming, movement-first stock, and explicit storefront publication. See `docs/superpowers/specs/2026-07-12-curate-inventory-ingestion-design.md` and the implementation plan for rollout evidence.
