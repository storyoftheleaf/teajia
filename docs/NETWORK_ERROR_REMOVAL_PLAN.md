...
Implementation note for 2.1: the proxies now use a 12 second read budget and a 25 second body-bearing request budget, both shorter than their matching client attempt. Unit coverage verifies the timeout response and signal wiring. The required real multi-megabyte upload through a preview deployment remains pending, so 2.1 stays unchecked.
