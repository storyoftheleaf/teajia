...
- [ ] **Check whether the sales inquiry inbox is actually reachable without signing in, then re-land the sixteen security fixes stranded on `codex/sales-system-rebuild`.** _(band: you-required to decide, then agent-runnable)_ _(effort: deep)_ → Plan: [sales-rebuild-salvage.md](todo/plans/sales-rebuild-salvage.md)
  Audited 2026-08-21. That branch is two things: sixteen finished security and correctness fixes to the invoice and inquiry flow that are genuinely absent from main, and a nine-task plan for a new sales architecture that was never started and whose migration numbering main has since taken. The architecture half is dead; the fixes are not. Several are access-control fixes — one of them is titled "authenticate the sales inquiry inbox," which implies it currently is not. Confirming that on the live admin route is one look and decides whether this is cleanup or this week.
