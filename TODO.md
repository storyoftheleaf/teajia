...
- [ ] **The browser suite cannot start at all from a worktree: the runner loads a different Playwright install than the tests do.** _(band: agent-runnable)_ _(effort: moderate)_
  Measured 2026-08-28. Every run dies before the first test with "did not expect test.describe() to be called here / two different versions of @playwright/test", and the stack frames come from `.claude/worktrees/shop-section-cleanup-163ca9/node_modules/playwright` while `require.resolve` from the spec folder correctly gives the repo's own copy. Three separate installs sit in the ancestor chain: the repo, that sibling worktree, and the workspace root at `Coding/node_modules`, whose `.bin/playwright` is what `npx` finds when it walks up. It reproduces identically from the main checkout with no branch code involved, so it is environmental, not a regression. Pinning the CLI by absolute path and forcing a private TMPDIR both failed to change it, so the fix is to remove the duplicate installs rather than to work around resolution.

- [ ] **The shared node_modules is behind main, and a worktree dev server currently fails to boot because of it.** _(band: routine)_ _(effort: quick)_
  Measured 2026-08-28. `concurrently`, added to package.json by the shop commit on main, is not present in `node_modules`, which every worktree shares by symlink. A dev server started from a worktree on the rebased branch crashes at boot with "Invalid hook call / more than one copy of React" in ThemeProvider, though only one React exists on disk. Not verified against the main checkout's own server, so the blast radius is unmeasured. An `npm install` is the likely fix, but it mutates state shared with whatever sessions are running, so it wants a quiet moment rather than a background command.
