...
- [ ] **The payment ledger rules are written twice and have to be changed twice.** _(band: agent-runnable)_ _(effort: moderate)_
  `worker/src/index.ts` imports `worker/src/mcp.ts`, so mcp.ts cannot import back without closing a module cycle. To build the voice tools in round three, eleven pieces of the money code were copied into mcp.ts and marked "KEEP IN SYNC": `roundUsd`, `PAYMENT_EPSILON`, `PAYMENT_TEXT_LIMITS`, `paymentTextField`, `loadInvoiceLedgerTotals`, `invoiceMoney`, `loadLedgerInvoice`, `reconcileLedgerWithColumn`, `recomputeInvoicePaymentStatus`, the buyer case of `ensureContactRelationship`, and the whole of `handleConvertInquiry`'s line resolution and invoice-number allocation. A change to what counts as paid now has to be made in both files or the screen and the spoken answer quietly disagree about money. `worker/src/invoiceDomain.ts` already exists with no dependency on index.ts, so the five ledger functions can move there and both files import them; the mcp.ts call sites are already shaped for it. The same pass should drop the `status = 'Draft'` requirement from the zero-price half of `whats_waiting`'s unpriced query, which is the one place `/api/attention` and `whats_waiting` currently differ (recorded in `todo/plans/order-process-notes.md`).
